...
def clear_database():
    ...
    try:
        ...
        if wal_path.exists():
            os.remove(wal_path)
            print(f"✅ Удален файл: {wal_path.name}")
        
        if shm_path.exists():
            os.remove(shm_path)
            print(f"✅ Удален файл: {shm_path.name}")
        
        print(f"\n✨ База данных успешно очищена!")
        print(f"📝 Резервная копия сохранена: {backup_path}")
        print(f"\n💡 При следующем запуске приложения будет создана новая чистая база данных")
        
        # Показываем размер бэкапа
        backup_size_mb = backup_path.stat().st_size / (1024 * 1024)
        print(f"📊 Размер резервной копии: {backup_size_mb:.2f} MB")
...
def restore_database(backup_name: str = None):
    ...
    if not db_backup_dir.exists():
        print(f"❌ Директория с резервными копиями не найдена: {db_backup_dir}")
        return
    ...
    if not all_backups:
        print(f"❌ Резервные копии не найдены в {db_backup_dir}")
        return
    ...
    if backup_name:
        ...
        print(f"📦 Используется последняя резервная копия: {backup_path.name}")
    ...
    try:
        ...
        if db_path.exists():
            ...
            print(f"📦 Создана резервная копия текущей БД: {temp_backup.name}")
        
        # Восстанавливаем из бэкапа
        print(f"\n🔄 Восстановление базы данных...")
        shutil.copy2(backup_path, db_path)
        print(f"✅ База данных восстановлена из {backup_path.name}")
...
if __name__ == "__main__":
    ...
    if args.action == "clear":
        ...
        print("⚠️  ВНИМАНИЕ: Будет создана резервная копия, затем база данных будет удалена!")
