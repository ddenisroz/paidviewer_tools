...
class GPUConfig:
    ...
    def from_env(cls) -> 'GPUConfig':
        ...
        return cls(
            # Redis настройки
            redis_url=os.getenv('GPU_REDIS_URL', 'redis://localhost:6379/0'),
            input_stream=os.getenv('GPU_INPUT_STREAM', 'tts_requests'),
            output_stream=os.getenv('GPU_OUTPUT_STREAM', 'tts_results'),
            gpu_stream=os.getenv('GPU_STREAM', 'gpu_tts_requests'),
            gpu_results_stream=os.getenv('GPU_RESULTS_STREAM', 'gpu_tts_results'),
            consumer_group=os.getenv('GPU_CONSUMER_GROUP', 'gpu_workers'),
            
            # Настройки concurrency
            min_concurrent=int(os.getenv('GPU_MIN_CONCURRENT', '1')),
            max_concurrent=int(os.getenv('GPU_MAX_CONCURRENT', '4')),
            initial_concurrent=int(os.getenv('GPU_INITIAL_CONCURRENT', '2')),
            
            # Пороги GPU памяти
            gpu_memory_threshold_low=float(os.getenv('GPU_MEMORY_THRESHOLD_LOW', '0.3')),
            gpu_memory_threshold_high=float(os.getenv('GPU_MEMORY_THRESHOLD_HIGH', '0.8')),
            gpu_memory_threshold_critical=float(os.getenv('GPU_MEMORY_THRESHOLD_CRITICAL', '0.9')),
            
            # Настройки мониторинга
            monitoring_interval=float(os.getenv('GPU_MONITORING_INTERVAL', '5.0')),
            health_history_size=int(os.getenv('GPU_HEALTH_HISTORY_SIZE', '100')),
            
            # Настройки задач
            task_timeout=float(os.getenv('GPU_TASK_TIMEOUT', '300.0')),
            batch_size=int(os.getenv('GPU_BATCH_SIZE', '10')),
            retry_attempts=int(os.getenv('GPU_RETRY_ATTEMPTS', '3')),
            retry_delay=float(os.getenv('GPU_RETRY_DELAY', '5.0')),
        )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'gpu_results_stream': self.gpu_results_stream,
            'consumer_group': self.consumer_group,
            'min_concurrent': self.min_concurrent,
            'max_concurrent': self.max_concurrent,
            'initial_concurrent': self.initial_concurrent,
            'gpu_memory_threshold_low': self.gpu_memory_threshold_low,
            'gpu_memory_threshold_high': self.gpu_memory_threshold_high,
            'gpu_memory_threshold_critical': self.gpu_memory_threshold_critical,
