...
### ✨ Drops System улучшения

#### OBS Widget токен
- **Новая миграция**: `add_widget_token` — добавлено поле `widget_token` в `drops_configs`
  - Уникальный токен для идентификации канала в OBS виджете без авторизации
  - Индекс для быстрого поиска по токену
  - Генерация токена при создании URL виджета
- **Перегенерация токена**: добавлена кнопка "Перегенерировать" в WidgetSettings
  - Endpoint: `POST /api/drops/widget-url?regenerate=true`
  - Безопасное обновление токена с предупреждением об обновлении URL в OBS
- **Улучшенная анимация виджета**:
  - Открытие сундука (вместо вращения)
  - Рулетка наград поверх открытого сундука
  - Правильное отображение карточек наград из соответствующего качества

#### DonationAlerts интеграция
- **Автоматическое подключение**: при включении "Включить donation drops" автоматически запускается подключение DonationAlerts
- Убрана блокировка переключателя - теперь всегда доступен для включения

### 🎮 Commands System исправления

#### Иерархия ролей
- **Исправлена логика проверки ролей** в `command_executor.py`:
  - `all` — доступно всем (включая VIP, модераторов, владельца)
  - `vip` — доступно VIP, модераторам и владельцу
  - `moderator` — доступно модераторам и владельцу
  - `broadcaster` — только владелец
- **Исправлено отображение пустых данных** в карточках команд:
  - Если `allowed_roles` пустое или null — показывается "Все зрители"
  - Правильная обработка отсутствующих значений

### 🎨 UI/UX улучшения

#### Навигация Sidebar
- **Фон активных элементов доходит до края**:
  - Фон применен к самому внешнему элементу с `absolute inset-0`
  - Убран лишний padding с контейнера `nav`
  - Правильное позиционирование для элементов с подменю и обычных ссылок
- **Улучшено отображение текста**: убрана обрезка текста, разрешен перенос по словам

#### Admin Panel
- Исправлено отображение платформ в командах: "Все платформы" показывается корректно
- Убрана информация об "override" из описания настроек команды

### 🐛 API исправления

#### Points API
- **Исправлена расшифровка токенов** в `points_api_endpoints.py`:
  - Токены расшифровываются перед использованием в Twitch API
  - Токены расшифровываются перед использованием в VK API
  - Добавлена правильная обработка ошибок от Twitch API (403 для непартнеров)

#### Drops API
- **Безопасная обработка widget_token**:
  - Проверка наличия поля перед использованием (для обратной совместимости)
  - Безопасное создание конфигурации через DropsService
...
#### Исправления проблем миграции
- **admin/VoiceManagement.jsx** ✅:
  - Исправлено: убрано дублирование `useState` + React Query
  - Данные теперь используются напрямую из `voicesData` и `usersData`
  - Убраны все `setVoices()` вызовы, заменены на `queryClient.setQueryData()` или `invalidateQueries`
  - Добавлено детальное логирование для отладки проблем с загрузкой голосов
- **VoiceManagementPage.jsx** ✅:
  - Исправлено: убрано дублирование `useState` для `globalVoices` и `userVoices`
  - Данные используются напрямую из React Query через `globalVoicesData` и `userVoicesData`
  - Optimistic updates переведены на `queryClient.setQueryData()`
