...
```python
# websocket_helper.py (Refactored to use BlockedUserRepository)
from bot_service.repositories.blocked_user_repository import BlockedUserRepository
repo = BlockedUserRepository(db)
if repo.is_blocked(channel_name, platform, username):
...
```python
# Refactored to use BlockedBotRepository
from bot_service.repositories.blocked_bot_repository import BlockedBotRepository
if BlockedBotRepository(db).is_blocked(username):
...
```python
if use_ai_tts:
    from bot_service.repositories.whitelisted_channel_repository import WhitelistedChannelRepository
    ...
    if channel_owner.twitch_username:
        is_whitelisted = repo.is_whitelisted(channel_owner.twitch_username)
    
    # Проверяем VK
    if not is_whitelisted and channel_owner.vk_username:
        is_whitelisted = repo.is_whitelisted(channel_owner.vk_username)
...
is_whitelisted = False
if db_user.twitch_username and repo.is_whitelisted(db_user.twitch_username):
    is_whitelisted = True
elif db_user.vk_username and repo.is_whitelisted(db_user.vk_username):
    is_whitelisted = True
