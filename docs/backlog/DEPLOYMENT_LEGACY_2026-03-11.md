...
For the current TTS deployment contract, use these docs first:
...
5. `docs/setup/DOCKER_DEPLOYMENT.md`

Current source-of-truth rules:

- frontend talks only to `bot_service`
- advanced synthesis for `f5` and `qwen` goes through `tts-gateway`
- voice/admin APIs stay provider-owned
- `qwen` voice CRUD is intentionally disabled in this repo until `QWEN_VOICE_SERVICE_URL` is configured
- TTS upstream auth is strict API-key with `Authorization` and `X-API-Key`

Current compose baselines in this repo:

- `deploy/docker/docker-compose.dev.yml`
- `deploy/docker/docker-compose.bot.yml`
- `deploy/docker/docker-compose.prod.yml`

Legacy overlays such as `docker-compose.tts-advanced.yml` and `docker-compose.tts-simple.yml` require a separate audit before they are treated as current deployment source of truth.
