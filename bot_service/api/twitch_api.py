...
logger.info(f"[SEARCH] Environment check - TWITCH_CLIENT_ID loaded: {bool(settings.twitch_client_id)}")
logger.info(f"[SEARCH] Environment check - TWITCH_CLIENT_SECRET loaded: {bool(settings.twitch_client_secret)}")
logger.info(f"[SEARCH] Environment check - TWITCH_BOT_TOKEN loaded: {bool(settings.twitch_bot_token)}")
...
class TwitchAPI:
    ...
    async def update_stream_category(self, user_id: int, category_id: str) -> bool:
        ...
        logger.info(f"[GAME] [TWITCH API] Updating category for user {user_id} to category_id={category_id}")

        # Получаем токен пользователя (используем тот же метод что и update_stream_title)
        tokens = get_user_token_from_db(user_id, "twitch")
        if not tokens or not tokens.get("access_token"):
            logger.error(f"[ERROR] [TWITCH API] No Twitch access token found for unified user {user_id}")
            return False

        # Проверяем наличие необходимых прав в токене
        if "channel:manage:broadcast" not in tokens.get("scopes", []):
            logger.error(f"[ERROR] [TWITCH API] User {user_id} does not have 'channel:manage:broadcast' scope for updating category.")
            return False
        ...
        if not platform_user_id:
            logger.error(f"[ERROR] [TWITCH API] Could not find platform_user_id for unified user {user_id}")
            return False

        logger.info(f"[OK] [TWITCH API] Got access token and platform_user_id={platform_user_id} for user {user_id}")
        ...
        async with aiohttp.ClientSession(timeout=TWITCH_API_TIMEOUT) as session:
            ...
            logger.info("[GAME] [TWITCH API] Sending PATCH request:")
            logger.info(f"   URL: {url}")
            logger.info(f"   Data: {data}")

            async with session.patch(url, headers=headers, json=data) as response:
                logger.info(f"[GAME] [TWITCH API] Response status: {response.status}")

                if response.status == 204:
                    logger.info(f"[OK] [TWITCH API] Stream category updated successfully for user {user_id}")
                    return True
                elif response.status == 401:
                    error_text = await response.text()
                    logger.error(f"[ERROR] [TWITCH API] Token expired for user {user_id}: {response.status} - {error_text}")
                    # Попробуем обновить токен
                    try:
                        await self._refresh_user_token(user_id)
                        logger.info(f"[REFRESH] [TWITCH API] Attempting to refresh Twitch token for user {user_id}")
                        return False  # Возвращаем False, чтобы пользователь переавторизовался
                    except Exception as refresh_error:
                        logger.error(f"[ERROR] [TWITCH API] Failed to refresh Twitch token for user {user_id}: {refresh_error}")
                        return False
                else:
                    error_text = await response.text()
                    logger.error(f"[ERROR] [TWITCH API] Failed to update stream category for user {user_id}: {response.status}")
            ...
            async with aiohttp.ClientSession(timeout=TWITCH_API_TIMEOUT) as session:
                ...
                async with session.post(url, data=data) as response:
                    if response.status == 200:
                        ...
                        logger.info(f"[AUTH] [TWITCH REFRESH] Token expires_in: {expires_in} seconds ({expires_in / 3600:.1f} hours)")
                        ...
                        # [HOT] Инвалидируем кеш после обновления токена
                    ...
                    if response.status == 200:
                        ...
                        if response.status == 403:
                            try:
                                ...
                                raise ValueError("403:The broadcaster must have partner or affiliate status.")
                        return None
        except ValueError:
        ...
        try:
            ...
            async with aiohttp.ClientSession(timeout=TWITCH_API_TIMEOUT) as session:
                async with session.post(url, headers=headers, params=params, json=reward_data) as response:
                    if response.status == 200:
                        data = await response.json()
                        logger.info(f"[OK] Created Twitch custom reward: {reward_data.get('title')}")
                        ...
                        logger.error(f"[ERROR] Twitch create reward error [{response.status}]: {error_message}")
            ...
            logger.error(f"[ERROR] Error creating Twitch custom reward: {e}", exc_info=True)
        ...
        try:
            ...
            async with aiohttp.ClientSession(timeout=TWITCH_API_TIMEOUT) as session:
                async with session.patch(url, headers=headers, params=params, json=reward_data) as response:
                    if response.status == 200:
                        data = await response.json()
                        logger.info(f"[OK] Updated Twitch custom reward: {reward_id}")
                        return data.get("data", [{}])[0] if data.get("data") else None
                    else:
                        error_text = await response.text()
                        error_message = f"HTTP {response.status}"
                        ...
                        logger.error(f"[ERROR] Twitch update reward error [{response.status}]: {error_message}")
                        logger.error(f"   Full response: {error_text}")
            ...
            logger.error(f"[ERROR] Error updating Twitch custom reward: {e}", exc_info=True)
    ...
    async def delete_custom_reward(self, broadcaster_id: str, reward_id: str, access_token: str) -> bool:
        ...
        try:
            ...
            async with aiohttp.ClientSession(timeout=TWITCH_API_TIMEOUT) as session:
                async with session.delete(url, headers=headers, params=params) as response:
                    if response.status == 204:
                        logger.info(f"[OK] Deleted Twitch custom reward: {reward_id}")
                        return True
                    else:
                        error_text = await response.text()
                        error_message = f"HTTP {response.status}"
                        ...
                        logger.error(f"[ERROR] Twitch delete reward error [{response.status}]: {error_message}")
                        logger.error(f"   Full response: {error_text}")
            ...
            logger.error(f"[ERROR] Error deleting Twitch custom reward: {e}", exc_info=True)
        ...
        try:
            ...
            async with aiohttp.ClientSession(timeout=TWITCH_API_TIMEOUT) as session:
                async with session.post(url, headers=headers, params=params, json=body) as response:
                    if response.status == 200:
                        logger.info(f"[OK] [TWITCH TIMEOUT] User {target_username} timed out for {duration_seconds}s")
                        return True
                    else:
                        error_text = await response.text()
                        logger.error(f"[ERROR] [TWITCH TIMEOUT] Failed: {response.status} - {error_text}")
                        return False

        except Exception as e:
            logger.error(f"[ERROR] [TWITCH TIMEOUT] Error: {e}")
            import traceback
    ...
    async def untimeout_user(
        ...
        try:
            ...
            async with aiohttp.ClientSession(timeout=TWITCH_API_TIMEOUT) as session:
                async with session.delete(url, headers=headers, params=params) as response:
                    if response.status == 204:
                        logger.info(f"[OK] [TWITCH UNTIMEOUT] Timeout removed for {target_username}")
                        return True
                    else:
                        error_text = await response.text()
                        logger.error(f"[ERROR] [TWITCH UNTIMEOUT] Failed: {response.status} - {error_text}")
                        return False

        except Exception as e:
            logger.error(f"[ERROR] [TWITCH UNTIMEOUT] Error: {e}")
            import traceback
            logger.error(traceback.format_exc())
            return False
