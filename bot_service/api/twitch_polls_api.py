...
class PollCreate(BaseModel):
    ...
    title: str = Field(..., min_length=1, max_length=60, description='Вопрос голосования')
    choices: List[PollChoice] = Field(..., min_length=2, max_length=5, description='Варианты ответов (2-5)')
    duration: int = Field(..., ge=15, le=1800, description='Длительность в секундах (15-1800)')
    channel_points_voting_enabled: bool = Field(default=False, description='Разрешить голосование за Channel Points')
    channel_points_per_vote: Optional[int] = Field(default=None, ge=1, le=1000000, description='Стоимость голоса в Channel Points (1-1000000)')
    ...
    def validate_channel_points(cls, v: Optional[int], info) -> Optional[int]:
        ...
        if enabled and (not v):
            raise ValueError('channel_points_per_vote обязателен когда channel_points_voting_enabled=true')
        if not enabled and v:
            raise ValueError('channel_points_per_vote не должен быть указан когда channel_points_voting_enabled=false')
        return v

class PollEnd(BaseModel):
    """Завершение голосования"""
    status: Literal['TERMINATED', 'ARCHIVED'] = Field(..., description='Статус завершения (TERMINATED - досрочно, ARCHIVED - архивировать)')
...
async def get_twitch_token(user: User) -> str:
    ...
    try:
        ...
        if not user_token or not user_token.access_token:
            ...
            raise HTTPException(status_code=400, detail='Twitch OAuth токен не найден')
...
async def make_twitch_api_request(method: str, endpoint: str, token: str, json_data: Optional[dict]=None, params: Optional[dict]=None) -> dict:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f'Ошибка подключения к Twitch API')
...
async def create_poll(poll: PollCreate, current_user: User=Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f'Ошибка создания голосования')
...
async def end_poll(poll_id: str, end_data: PollEnd, current_user: User=Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f'Ошибка завершения голосования')
...
async def get_active_polls(current_user: User=Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f'Ошибка получения голосований')
...
async def get_poll(poll_id: str, current_user: User=Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f'Ошибка получения голосования')
