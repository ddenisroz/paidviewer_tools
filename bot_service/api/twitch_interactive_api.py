...
async def get_hype_train_events(first: int=Query(default=1, ge=1, le=100, description='Количество событий (1-100)'), current_user: User=Depends(get_current_user)):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f'Ошибка получения Hype Train')
...
async def get_current_hype_train(current_user: User=Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f'Ошибка получения текущего Hype Train')

@router.post('/clips/create', response_model=dict)
async def create_clip(has_delay: bool=Query(default=False, description='Добавить задержку перед созданием клипа (для синхронизации со стримом)'), current_user: User=Depends(get_current_user)):
    """
    ...
    try:
        ...
        return {'success': True, 'clip': clip_data, 'message': 'Клип создается. Он будет доступен через несколько секунд.'}
    except HTTPException:
        raise
    except Exception as e:
        logger.error('twitch_clip_create_error', user_id=current_user.id, error=str(e))
        raise HTTPException(status_code=500, detail=f'Ошибка создания клипа')

@router.get('/clips', response_model=dict)
async def get_clips(first: int=Query(default=20, ge=1, le=100, description='Количество клипов (1-100)'), started_at: Optional[str]=Query(default=None, description='Начало периода (RFC3339 format)'), ended_at: Optional[str]=Query(default=None, description='Конец периода (RFC3339 format)'), current_user: User=Depends(get_current_user)):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f'Ошибка получения клипов')
