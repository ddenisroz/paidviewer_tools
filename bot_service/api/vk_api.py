...
# [WARN] ВНИМАНИЕ: Это применяется только для apidev.live.vkvideo.ru (dev API VK)
...
class VKLiveAPI:
    ...
    def _get_user_token(self, user_id: str, session_id: Optional[str] = None) -> Optional[str]:
        ...
        """
        try:
            # [START] UNIFIED: Используем TokenManager для единообразного получения токенов
            ...
            async with aiohttp.ClientSession(timeout=VK_API_TIMEOUT) as session:
                ...
                async with session.get(url, headers=headers) as response:
                    if response.status == 200:
                        ...
                        logger.debug("[INFO] /v1/current_user returned 401. This is expected if using bot token instead of streamer token.")
                        return None
                    else:
    ...
    async def _update_stream(self, user_id: str, payload: Dict[str, Any], session_id: Optional[str] = None) -> bool:
        ...
        try:
            ...
            logger.info(f"[OK] [VK API] Token retrieved for user {user_id}")
            ...
            try:
                user_record = db.query(User).filter(User.id == int(user_id)).first()
                if not user_record or not user_record.vk_channel_name:
                    logger.warning(f"[ERROR] [VK API] Could not get channel URL for user {user_id} from database")
                    return False
                channel_url = user_record.vk_channel_name
                logger.info(f"[OK] [VK API] Using cached channel_url from DB: {channel_url}")
            finally:
            ...
            async with aiohttp.ClientSession(timeout=VK_API_TIMEOUT) as session:
                ...
                async with session.get(get_url, headers={"Authorization": f"Bearer {token}"}, params=get_params) as response:
                    if response.status == 200:
                        ...
                        if stream_object and stream_object.get("status") == "started":
                            ...
                            logger.info("[OK] [VK API] Found active stream in 'stream' object.")
                        else:
                            streams_array = data.get("data", {}).get("streams", [])
                            for s in streams_array:
                                if s.get("status") == "started":
                                    current_stream_data = s
                                    logger.info("[OK] [VK API] Found active stream in 'streams' array.")
                                    break

                if not current_stream_data:
                    logger.warning(f"[WARN] [VK API] Could not find an active stream for {user_id}. Will try to edit anyway (VK API will reject if not allowed).")
                    ...
                    if stream_info and stream_info.get("category_id"):
                        ...
                        logger.info(f"[OK] [VK API] Using saved stream data: title='{current_stream_data['title']}', category_id='{current_stream_data['category']['id']}'")
                    else:
                        logger.warning(f"[WARN] [VK API] No saved stream data for {user_id}. Will try to update anyway with provided payload.")
                ...
                if "category" in payload and "id" in payload["category"]:
                    ...
                    if "title" not in payload["category"]:
                        logger.info(f"📺 [VK API] [WARN] STARTING TO LOAD FULL CATEGORY FOR ID: {category_id}")
                        try:
                            ...
                            async with session.get(cat_url, headers=headers, params=cat_params) as cat_response:
                                if cat_response.status == 200:
                                    ...
                                    if full_category:
                                        ...
                                        logger.info(f"[OK] [VK API] Loaded full category: {final_payload['category']}")
                                    else:
                            ...
                            logger.error(f"[ERROR] [VK API] Failed to load category info: {e}")
                            final_payload["category"]["id"] = category_id
                    else:
                        ...
                        if cover_url:
                            ...
                            logger.info("📺 [VK API] Skipping empty cover_url")
                ...
                if not final_payload["category"]["id"]:
                    logger.error(f"[ERROR] [VK API] Cannot update stream for {user_id} because category ID is missing.")
                    return False
                ...
                async with session.post(post_url, headers=headers, json=post_data, params=post_params) as response:
                    ...
                    if response.status == 200:
                        logger.info(f"[OK] [VK API] Successfully updated VK Live stream for user {user_id}")
                        return True
                    else:
                        response_text = await response.text()
                        logger.error(f"[ERROR] [VK API] Stream edit for user {user_id} returned {response.status}")
    ...
    async def update_stream_category(self, user_id: str, category_data, session_id: Optional[str] = None) -> bool:
        ...
        if result:
            logger.info(f"[OK] [VK API] Category updated successfully for user {user_id}")
        else:
            logger.error(f"[ERROR] [VK API] Failed to update category for user {user_id}")
                                ...
                                if new_token:
                                    ...
                                    async with session.get(url, headers=headers, params=params) as retry_response:
                                        if retry_response.status == 200:
                                            ...
                                            logger.info(f"[OK] Refreshed VK categories after token update: {len(categories)} found")
                                            return
                                        else:
    ...
    async def get_channel_points_balance(self, channel_url: str, access_token: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # [WARN] Отключаем проверку SSL для dev API VK (apidev.live.vkvideo.ru)
