...
def _reward_to_dict(reward, quality_info: dict) -> dict:
    ...
    return {'id': reward.id, 'name': reward.name, 'description': reward.description, 'quality': quality_info, 'weight': reward.weight, 'reward_type': reward.reward_type, 'reward_value': reward.reward_value, 'image_url': reward.image_url, 'sound_file': reward.sound_file, 'sound_volume': reward.sound_volume, 'is_active': reward.is_active, 'created_at': reward.created_at, 'updated_at': reward.updated_at}
    ...
    try:
        ...
        try:
            ...
            if user_id and user_id != -1:
                cache_invalidation_event = {'type': 'cache_invalidate', 'cache_key': f'drops_rewards_{channel_name}', 'reason': 'drops_reward_created'}
                await get_memory_websocket_manager().send_to_user(user_id, cache_invalidation_event)
        except Exception as ws_error:
            logger.warning(f'Failed to send WebSocket notification: {ws_error}')
        return {'success': True, 'message': 'Operation completed.', 'data': {'id': reward.id, 'name': reward.name, 'quality': quality.name, 'reward_type': reward.reward_type, 'created_at': reward.created_at}}
    except HTTPException:
        raise
    except Exception:
...
async def update_drops_reward(reward_id: int, reward_data: DropsRewardUpdate, current_user: dict=Depends(get_current_user), db: Session=Depends(get_db)):
    ...
    try:
        ...
        try:
            ...
            user_id = current_user.get('id')
            if user_id and user_id != -1:
                cache_invalidation_event = {'type': 'cache_invalidate', 'cache_key': f'drops_rewards_{reward.channel_name}', 'reason': 'drops_reward_updated'}
                await get_memory_websocket_manager().send_to_user(user_id, cache_invalidation_event)
        except Exception as ws_error:
            logger.warning(f'Failed to send WebSocket notification: {ws_error}')
        return {'success': True, 'message': 'Operation completed.', 'data': {'id': reward.id, 'name': reward.name, 'updated_at': reward.updated_at}}
    except HTTPException:
        raise
    except Exception:
...
async def toggle_drops_reward(reward_id: int, toggle_data: DropsRewardToggle, current_user: dict=Depends(get_current_user), db: Session=Depends(get_db)):
    ...
    try:
        ...
        try:
            ...
            user_id = current_user.get('id')
            if user_id and user_id != -1:
                cache_invalidation_event = {'type': 'cache_invalidate', 'cache_key': f'drops_rewards_{reward.channel_name}', 'reason': 'drops_reward_toggled'}
                await get_memory_websocket_manager().send_to_user(user_id, cache_invalidation_event)
        except Exception as ws_error:
            logger.warning(f'Failed to send WebSocket notification: {ws_error}')
        return {'success': True, 'message': 'Reward status updated', 'data': {'id': reward.id, 'is_active': reward.is_active, 'updated_at': reward.updated_at}}
    except HTTPException:
        raise
    except Exception:
...
async def delete_drops_reward(reward_id: int, current_user: dict=Depends(get_current_user), db: Session=Depends(get_db)):
    ...
    try:
        ...
        try:
            ...
            user_id = current_user.get('id')
            if user_id and user_id != -1:
                cache_invalidation_event = {'type': 'cache_invalidate', 'cache_key': f'drops_rewards_{channel_name}', 'reason': 'drops_reward_deleted'}
                await get_memory_websocket_manager().send_to_user(user_id, cache_invalidation_event)
        except Exception as ws_error:
            logger.warning(f'Failed to send WebSocket notification: {ws_error}')
...
async def upload_reward_image(reward_id: int, image_file: UploadFile=File(...), current_user: dict=Depends(get_current_user), db: Session=Depends(get_db)):
    ...
    try:
        ...
        invalidate_cache(f"drops_rewards:{current_user['id']}:{reward.channel_name}:")
        return {'success': True, 'message': 'Operation completed.', 'data': {'image_url': image_url, 'filename': filename}}
    except HTTPException:
        raise
    except Exception:
