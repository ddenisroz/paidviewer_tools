...
async def add_video_to_queue(request: AddVideoRequest, user: dict=Depends(get_current_user_optional), db: Session=Depends(get_db)):
    ...
    if not is_valid:
        ...
        for candidate in search_results:
            ...
            break
        if not selected_url:
            raise HTTPException(status_code=400, detail='No suitable video found for this query. Try another query.')
    ...
    logger.debug(f'[OK] [YOUTUBE] Valid URL: {video_input}Text cleaned.{video_id}')
    ...
    if len(current_queue) >= MAX_YOUTUBE_QUEUE_SIZE:
        ...
        raise HTTPException(status_code=400, detail=f'Queue is full (max {MAX_YOUTUBE_QUEUE_SIZE} videos). Please remove some videos first.')
    try:
        result = await queue_service.add_video_to_queue(user_id=user_id, session_id=None, video_url=video_input, channel_name='web_interface', platform='web', requester_name=f'User_{user_id}', requester_id=str(user_id), is_paid=request.is_paid, points_cost=request.points_cost, db=db)
        if result['success']:
            await notify_queue_update(user_id=user_id, session_id=None, db=db)
            response = {'success': True, 'message': 'Operation completed.', 'queue_item': result['queue_item']}
...
async def skip_to_next_video(user: dict=Depends(get_current_user), db: Session=Depends(get_db)):
    ...
    try:
        ...
        return {'success': True, 'message': 'Operation completed.', 'current_video': current_video}
    except HTTPException:
        raise
    except Exception:
...
async def ban_queue_item(queue_id: int, user: dict=Depends(get_current_user), db: Session=Depends(get_db)):
    ...
    try:
        ...
        if not result.get('success'):
            raise HTTPException(status_code=404, detail='Video not found in queue')
        await notify_queue_update(user['id'], db=db)
        return {'success': True, 'message': 'Video banned', 'video_id': result.get('video_id'), 'banned_count': result.get('banned_count', 0)}
    except HTTPException:
        raise
    except Exception:
...
async def clear_queue(user: dict=Depends(get_current_user), db: Session=Depends(get_db)):
    ...
    try:
        ...
        await notify_queue_update(user['id'], db)
        return {'success': True, 'message': f'Operation completed.{cleared_count}Operation completed.'}
    except HTTPException:
        raise
    except Exception:
