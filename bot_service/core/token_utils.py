...
async def validate_platform_token(token) -> bool:
    ...
    if cached_result is not None:
        logger.info(f"🚀 [CACHE HIT] Token validation for user {token.user_id}, platform {token.platform}: {cached_result}")
    ...
    try:
        ...
        # Кешируем результат
        token_validation_cache.set(token.user_id, token.platform, is_valid)
        logger.info(f"✅ VALIDATE_TOKEN END: platform={token.platform}, user_id={token.user_id}, valid={is_valid}")
        return is_valid
