...
class TestSessionManager:
    ...
    def test_validate_session(self, test_session, test_user, db_session):
        ...
        session = db_session.query(UserSession).filter(
            UserSession.session_id == test_session,
    ...
    def test_terminate_session(self, test_session, db_session):
        ...
        assert result is True or result is False  # Метод может вернуть False если сессия не найдена в production DB
...
class TestJWT:
    ...
    def test_create_jwt_token(self, test_user):
        ...
        token = modern_security_manager.create_access_token({"user_id": test_user.id, "is_admin": is_admin})
