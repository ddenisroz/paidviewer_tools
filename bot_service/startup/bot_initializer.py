...
async def initialize_vk_bot() -> bool:
    ...
    # Приоритет 2: ClientCredentials/Env токен (fallback)
    # NOTE: generate_vk_client_credentials_token() updates os.environ, so we must check it first
    vk_bot_token = os.environ.get("VK_LIVE_USER_TOKEN") or settings.vk_live_user_token
    
    if vk_bot_token:
        # Note: We trust the token if it exists (especially if generated by Client Credentials)
        # Validation against current_user might fail for App Tokens, so we skip strict check here.
        logger.info("[OK] Using VK Live ClientCredentials/Env token (Fallback)")
        try:
            bot = VKLiveBot(vk_bot_token, connection_manager)
            task = asyncio.create_task(bot.start_bot())
            registry.vk_bot = bot
            registry.vk_task = task
            logger.info("[OK] VK Live bot initialized with ClientCredentials/Env")
            return True
        except Exception as e:
            logger.error(f"[ERROR] VK bot creation failed: {e}")
            # Continue to next priority
    else:
        logger.warning("[WARN] No VK_LIVE_USER_TOKEN found in Env or Settings")

    # Приоритет 3: OAuth токен пользователя (Legacy/Streamer mode)
    await _connect_vk_bot_to_user_channel(registry, connection_manager)
    
    final_status = registry.is_vk_running()
    if not final_status:
         logger.warning("[WARN] Could not initialize VK Bot (Tried: BotToken, .env, UserToken)")
