...
def cleanup_users(db, keep_admins=True):
    ...
    if keep_admins:
        ...
        print(f"📋 Удаление пользователей (сохраняем админов)...")
    else:
        users_to_delete = db.query(User).all()
        print(f"📋 Удаление ВСЕХ пользователей (включая админов)...")
    ...
    db.commit()
    print(f"\n✅ Удалено пользователей: {len(users_to_delete)}")
    
    return deleted_counts

def main():
    print("\n" + "="*60)
    print("🗑️  СКРИПТ ОЧИСТКИ БАЗЫ ДАННЫХ ОТ ПОЛЬЗОВАТЕЛЕЙ")
    print("="*60)
    ...
    try:
        ...
        print("\n⚠️  ВНИМАНИЕ! Эта операция удалит ВСЕ данные пользователей!")
        ...
        confirm = input("\n⚠️  Вы уверены? Введите 'YES' для подтверждения: ").strip()
