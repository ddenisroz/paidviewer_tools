...
def check_token():
    ...
    if not settings.twitch_bot_token:
        print("❌ TWITCH_BOT_TOKEN is NOT configured in .env file")
        print("\nTo fix:")
    ...
    if not token.startswith("oauth:"):
        print("❌ Token does NOT start with 'oauth:' prefix")
    ...
    if token_length < 30:
        print(f"⚠️  Token seems too short: {token_length} characters")
