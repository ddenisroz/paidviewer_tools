...
def check_tts_status(user_id: int):
    ...
    try:
        ...
        if tts_settings:
            ...
            print(f"\n⚠️ No TTS settings found (will use defaults)")
        ...
        if user.twitch_username:
            twitch_wl = db.query(WhitelistedChannel).filter(
            ...
            if twitch_wl:
                ...
                print(f"❌ Twitch '{user.twitch_username}' NOT in whitelist")
        
        if user.vk_username:
            vk_wl = db.query(WhitelistedChannel).filter(
            ...
            if vk_wl:
                ...
                print(f"❌ VK username '{user.vk_username}' NOT in whitelist")
        
        if user.vk_channel_name:
            vk_ch_wl = db.query(WhitelistedChannel).filter(
            ...
            if vk_ch_wl:
                ...
                print(f"❌ VK channel '{user.vk_channel_name}' NOT in whitelist")
        
        print(f"\n=== Summary ===")
        if not user.tts_enabled:
            print(f"❌ TTS is DISABLED globally for this user")
            ...
            print(f"⚠️ User wants F5-TTS but NOT in whitelist")
