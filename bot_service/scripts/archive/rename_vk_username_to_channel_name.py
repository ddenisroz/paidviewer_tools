...
def migrate_vk_username_to_channel_name():
    ...
    try:
        with engine.connect() as conn:
            ...
            logger.info(f"[STATS] Существующие колонки: {columns}")

            if 'vk_username' in columns:
                logger.info("[REFRESH] Переименовываем vk_username → vk_channel_name")
                ...
                conn.commit()
                logger.info("[OK] Создана колонка vk_channel_name")

                # 2. Копируем данные
                conn.execute(text("UPDATE users SET vk_channel_name = vk_username WHERE vk_username IS NOT NULL"))
                conn.commit()
                logger.info("[OK] Данные скопированы")

                # 3. Удаляем старую колонку (для SQLite нужно пересоздать таблицу)
                # Проще оставить старую колонку, но использовать новую
                logger.info("[WARN] Старая колонка vk_username сохранена для совместимости")
                logger.info("[INFO] Используйте vk_channel_name в новом коде")

            elif 'vk_channel_name' in columns:
                logger.info("[INFO] Колонка vk_channel_name уже существует")
            else:
                logger.error("[ERROR] Ни vk_username, ни vk_channel_name не найдены!")
                return
