#!/usr/bin/env python3
"""
[WARN] ЧАСТИЧНО УСТАРЕВШИЙ СКРИПТ [WARN]

Функция fix_user_settings() - [OK] АКТУАЛЬНА
Функция init_commands_for_user() - [ERROR] УСТАРЕЛА (создает дублирующиеся 'basic' команды)
    ...
    try:
        ...
        for user in users:
            ...
            if not settings:
                ...
                if not channel_name:
                    print(f"[WARN]  User {user.id} has no username, skipping")
                    continue
                ...
                print(f"[OK] Created UserSettings for user {user.id} ({channel_name})")
                fixed += 1

        if fixed > 0:
            db.commit()
            print(f"\n[OK] Fixed {fixed} users")
        else:
            print("\n[OK] All users already have UserSettings")
...
def init_commands_for_user(user_id: int):
    """
    [WARN] УСТАРЕВШАЯ ФУНКЦИЯ - НЕ ИСПОЛЬЗОВАТЬ!
    ...
    """
    print("=" * 70)
    print("[WARN]  ФУНКЦИЯ УСТАРЕЛА И НЕ ДОЛЖНА ИСПОЛЬЗОВАТЬСЯ!")
    print("=" * 70)
    print()
    print(f"[ERROR] Создание команд типа 'basic' для user {user_id} приводит к дубликатам.")
    print("[OK] Глобальные команды уже доступны всем пользователям.")
    ...
    try:
        ...
        if not settings:
            print(f"[ERROR] User {user_id} has no UserSettings. Run fix_user_settings first.")
            return
        ...
        if created > 0:
            db.commit()
            print(f"[OK] Created {created} commands for user {user_id}")
        else:
            print(f"[OK] User {user_id} already has all commands")
...
if __name__ == "__main__":
    print("[FIX] Fixing existing users...")
    print("\n1. Creating UserSettings...")
    fix_user_settings()

    print("\n[WARN]  Skipping commands initialization (use init_global_commands.py instead)")
    print("\n[OK] Done!")
    print("\n[INFO] Tip: Run 'python init_global_commands.py' to ensure global commands exist")
