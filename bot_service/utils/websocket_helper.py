...
async def broadcast_chat_message(
    ...
    try:
        ...
        logger.info(f"[BROADCAST] [BROADCAST] Incoming message: {platform}:{channel} | {username}: {content[:50]}")
            ...
            try:
                ...
                if platform == 'twitch':
                    ...
                    if channel_owner:
                        ...
                        logger.debug(f"[OK] Found channel owner: user_id={user_id} for twitch channel {channel}")
                    else:
                        logger.warning(f"[WARN] Could not find Twitch channel owner for: {channel}")
                elif platform == 'vk':
                    ...
                    if channel_owner:
                        user_id = channel_owner.id
                        logger.debug(f"[OK] Found channel owner: user_id={user_id} for VK channel {channel}")
                    else:
                        logger.warning(f"[WARN] Could not find VK channel owner for: {channel}")

                # Если нашли владельца канала, сохраняем сообщение
                if user_id:
                    logger.info(f"[DB] [DB] Saving message: user_id={user_id}, channel={channel}, platform={platform}, author={username}, role={role}")
                    try:
                        ...
                        db.commit()
                        logger.info(f"[DB] [DB] Message saved: ID={chat_message.id}, {platform}:{channel} from {username} (role={role}, badges={badges})")
                        ...
                        if 'UniqueViolation' in str(db_error) or 'duplicate key' in str(db_error).lower():
                            logger.warning("[WARN] [DB] ID conflict detected, fixing sequence and retrying...")
                            db.rollback()
                            ...
                            try:
                                ...
                                db.execute(text("SELECT setval('chat_messages_id_seq', :new_val, false)"), {"new_val": max_id + 1})
                                db.commit()

                                logger.info(f"[OK] [DB] Sequence fixed: set to {max_id + 1}")
                                ...
                                db.add(chat_message)
                                db.commit()
                                logger.info(f"[DB] [DB] Message saved after retry: ID={chat_message.id}, {platform}:{channel} from {username}")
                            except Exception as seq_error:
                                logger.error(f"[ERROR] [DB] Failed to fix sequence: {seq_error}")
                                db.rollback()
                                # Продолжаем без сохранения в БД, но отправляем сообщение
                        else:
                            # Другая ошибка - логируем и продолжаем
                            logger.error(f"[ERROR] [DB] Unexpected error saving message: {db_error}")
                            db.rollback()
                else:
                    logger.warning(f"[WARN] [DB] Could not find channel owner for {platform}:{channel}, skipping DB save")
            finally:
                db.close()
        except Exception as e:
            logger.error(f"[ERROR] [DB] Failed to save message: {e}", exc_info=True)

        # Отправляем всем подключенным клиентам
        total_connections = len(memory_websocket_manager.connections)
        logger.info(f"[BROADCAST] [BROADCAST] Total WebSocket connections: {total_connections}")

        if total_connections == 0:
            logger.debug(f"[WARN] No WebSocket connections available for {platform} message")
            return False

        # [OK] ОПТИМИЗАЦИЯ: Batch отправка для лучшей производительности
        sent_count = 0
        ...
        async def send_to_connection(conn_id: str, conn):
            try:
                ...
                logger.error(f"[ERROR] Failed to send {platform} message to {conn_id}: {e}")
                return False
        ...
        logger.info(f"[SEND] {platform.upper()} message sent to {sent_count}/{total_connections} connections")
        return sent_count > 0

    except Exception as e:
        logger.error(f"[ERROR] WebSocket broadcast error for {platform}: {e}")
        return False
...
async def handle_tts_for_message(
    ...
    try:
        ...
        # [OK] ВАЖНО: is_tts_enabled() определяет ПОПЫТАЕМСЯ ЛИ МЫ AI TTS
        ...
        if connection_manager:
            ...
            logger.info(f"[DEBUG] [{platform.upper()} TTS] connection_manager.is_tts_enabled({channel_identifier}) = {use_ai_tts_requested}")
            logger.info(f"[DEBUG] [{platform.upper()} TTS] Will try AI TTS: {use_ai_tts_requested}")
        else:
        ...
        try:
            ...
            # [OK] ОПТИМИЗАЦИЯ: Используем кешированную проверку
            ...
            # [OK] КРИТИЧЕСКИ ВАЖНО: Проверяем включен ли ГЛОБАЛЬНО TTS для пользователя!
            ...
            # [OK] ОПТИМИЗАЦИЯ: Загружаем все настройки одним запросом с JOIN
            ...
            # [OK] НОВАЯ ЛОГИКА: Проверяем режим TTS (все сообщения / за баллы)
            if tts_user_settings.tts_mode == 'channel_points':
                ...
                if platform not in tts_reward_ids:
                    logger.warning(f"[ERROR] [{platform.upper()} TTS] No TTS reward configured for {platform}")
                    return {"success": False, "error": f"TTS reward not configured for {platform}"}

                expected_reward_id = tts_reward_ids[platform]
                logger.info(f"[POINTS] [{platform.upper()} TTS] Expected reward ID: {expected_reward_id}")

                # Проверяем что сообщение пришло с правильной наградой
                if not reward_id:
                    logger.warning(f"[ERROR] [{platform.upper()} TTS] Message not from reward redemption, skipping")
                ...
                logger.info(f"[OK] [{platform.upper()} TTS] Message from correct TTS reward! Processing...")

            # [OK] НОВАЯ ЛОГИКА: Определяем главный engine, но ВСЕГДА включаем fallback
            use_ai_tts = (tts_user_settings.engine == 'f5tts')
            use_basic_tts = True  # [OK] ВСЕГДА включаем базовую TTS как fallback (даже если engine='f5tts')

            # [OK] ПРИОРИТЕТ: use_ai_tts_requested из connection_manager может ОТКЛЮЧИТЬ AI TTS
            ...
            # [OK] ВАЖНО: Если пользователь не выбрал explicit engine,
            # или engine не установлен - ИСПОЛЬЗУЕМ БАЗОВУЮ TTS КАК DEFAULT
            if not use_ai_tts and not use_basic_tts:
                logger.info(f"[MIC] [{platform.upper()} TTS] Engine not set, using basic TTS as default")
                use_basic_tts = True
            ...
            # [OK] ОПТИМИЗАЦИЯ: Проверяем фильтры слов и применяем их эффективнее
            ...
            # [OK] ОПТИМИЗАЦИЯ: Применяем фильтры к тексту более эффективно
            filtered_text = text
            if filtered_words:
                import re
            ...
            if is_blocked:
                logger.info(f"[BLOCKED] User {username} is blocked from TTS by owner")
                db.close()
                return {"success": False, "error": "User is blocked from TTS"}

            # [SHIELD] ФИЛЬТР ОТВЕТОВ (если включен)
            if tts_user_settings.filter_replies and is_reply:
                logger.info(f"[SKIP] [{platform.upper()} TTS] Skipping reply message (filter_replies=True)")
                db.close()
                return {"success": False, "error": "Reply messages are filtered"}

            # [SHIELD] ФИЛЬТР УПОМИНАНИЙ (если включен)
            if tts_user_settings.filter_mentions and mentioned_users:
                ...
                if has_mentions:
                    logger.info(f"[SKIP] [{platform.upper()} TTS] Skipping message with mentions (filter_mentions=True)")
                    db.close()
            ...
            # [START] FIX: Если персональных настроек нет, не передаем voice_settings вообще,
            ...
            tts_settings_dict = {
                ...
                "voice": tts_user_settings.voice  # [OK] Передаем голос пользователя
            }

            # [START] FIX: Базовая громкость из AudioSettings (дефолт от админа/системы)
            ...
            # [START] FIX: Загружаем персональные настройки голоса и применяем volume
            ...
            if use_ai_tts and tts_user_settings.voice:
                ...
                if user_voice_config:
                    # [OK] Передаем персональные настройки только если они есть
                    # Если их нет, tts_engine.py использует дефолтные из таблицы Voice
                    voice_settings_dict = {}

                    # [OK] cfg_strength: персональный или None (будет использован дефолт из Voice)
                    if user_voice_config.cfg_strength is not None:
                        voice_settings_dict["cfg_strength"] = user_voice_config.cfg_strength

                    # [OK] speed_preset: персональный или None (будет использован дефолт из Voice)
                    if user_voice_config.speed_preset is not None:
                        voice_settings_dict["speed_preset"] = user_voice_config.speed_preset

                    # [OK] volume обрабатывается отдельно через final_volume_level (не передаем в voice_settings)
                    ...
                    if voice_settings_dict:
                        ...
                        logger.info(f"[SETTINGS] [{platform.upper()} TTS] Using personal voice settings: {voice_settings_dict}")
                else:
                    # [OK] Персональных настроек нет - tts_engine.py использует дефолты из Voice таблицы
                    logger.debug(f"[SETTINGS] [{platform.upper()} TTS] No personal voice settings found, will use defaults from Voice table")

            # [START] FIX: Финальная громкость: персональный volume из voice_settings или базовый
            ...
            if user_voice_config and user_voice_config.volume is not None:
                ...
                logger.debug(f"[VOLUME] [{platform.upper()} TTS] Using personal volume: {final_volume_level}% (base: {base_volume_level}%)")
            else:
                logger.debug(f"[VOLUME] [{platform.upper()} TTS] Using base volume: {final_volume_level}%")
            ...
            result = await tts_api.send_tts_request(
                ...
                author=username,
                user_id=user_id,
                volume_level=final_volume_level,  # [OK] Используем финальную громкость (персональную или базовую)
            ...
            if result.get("success"):
                ...
                logger.info(f"[OK] [{platform.upper()} TTS] Message queued: {username} ({tts_type})")
                logger.info(f"[OK] [{platform.upper()} TTS] Audio URL: {result.get('audio_url')}")
                logger.info(f"[OK] [{platform.upper()} TTS] Voice: {result.get('voice', 'unknown')}")
                logger.info(f"[OK] [{platform.upper()} TTS] Duration: {result.get('duration', 0)}s")

                # [OK] Автоматическое принятие наград TTS для VK и Twitch
                if reward_id and tts_user_settings.tts_mode == 'channel_points':
                    try:
                        ...
                        if platform == 'vk':
                            # [OK] Автоматическое принятие для VK
                                    ...
                                    if demands_data and isinstance(demands_data, dict):
                                        ...
                                        if isinstance(demands_list, list):
                                            ...
                                            if tts_demands:
                                                ...
                                                if demand_ids:
                                                    # [OK] Автоматически принимаем награды TTS
                                                    ...
                                                    if accept_result:
                                                        logger.info(f"[OK] [VK TTS] Auto-accepted {len(demand_ids)} TTS reward demands")
                                                    else:
                                                        logger.warning("[WARN] [VK TTS] Failed to auto-accept TTS reward demands")

                        elif platform == 'twitch':
                            # [OK] Автоматическое принятие для Twitch
                                    ...
                                    if redemptions and isinstance(redemptions, list):
                                        ...
                                        if target_redemptions:
                                            ...
                                            if latest_redemption_id:
                                                # [OK] Автоматически принимаем награду TTS
                                                ...
                                                if fulfill_result:
                                                    logger.info(f"[OK] [TWITCH TTS] Auto-fulfilled TTS reward redemption: {latest_redemption_id} for {username}")
                                                else:
                                                    logger.warning("[WARN] [TWITCH TTS] Failed to auto-fulfill TTS reward redemption")
                                except Exception as twitch_error:
                                    logger.warning(f"[WARN] [TWITCH TTS] Error getting/fulfilling redemptions: {twitch_error}")
                    except Exception as auto_accept_error:
                        logger.warning(f"[WARN] [{platform.upper()} TTS] Error auto-accepting reward: {auto_accept_error}")
                ...
                audio_data={
                    ...
                    "volume": final_volume_level,  # [OK] Исправлено: было volume_level
                    "tts_type": tts_type,
                ...
                logger.error(f"[ERROR] [{platform.upper()} TTS] Synthesis FAILED")
                logger.error(f"[ERROR] [{platform.upper()} TTS] Result: {result}")
                logger.error(f"[ERROR] [{platform.upper()} TTS] Error: {result.get('error')}")
        ...
        logger.error(f"[ERROR] [{platform.upper()} TTS] Error processing TTS: {e}")
        import traceback
        logger.error(f"[ERROR] [{platform.upper()} TTS] Traceback: {traceback.format_exc()}")
...
async def broadcast_tts_audio(
    ...
    try:
        ...
        if total_connections == 0:
            logger.debug("[WARN] No WebSocket connections for TTS audio playback")
            return False
        ...
        async def send_to_connection(conn_id: str, conn):
            try:
                ...
                logger.error(f"[ERROR] Failed to send TTS audio to {conn_id}: {e}")
                return False
        ...
        logger.info(f"[VOLUME] TTS audio sent to {sent_count}/{total_connections} connections (voice={audio_data.get('voice')})")
        return sent_count > 0
...
async def broadcast_drops_event(drops_data: Dict[str, Any]) -> bool:
    ...
    try:
        ...
        logger.info(f"[REWARD] [DROPS] Broadcasted drops event: {drops_data.get('viewer_name')} got {drops_data.get('reward')}")
        return True
