"""
OAuth авторизация для Twitch бота с поддержкой refresh_token.
...
async def login_twitch_bot(request: Request):
    """
    Инициировать OAuth авторизацию для Twitch бота.
    
    Требует права администратора.
    Перенаправляет на Twitch для авторизации бота.
    """
    try:
        ...
        # Сохраняем state в сессии (можно использовать Redis)
...
async def twitch_bot_callback(
    ...
    """
    Обработка OAuth callback для Twitch бота.
    
    Получает токены и сохраняет их в базу данных.
    """
    ...
    try:
        ...
        # 4. Перезапускаем бота с новым токеном
        ...
        # Запускаем нового бота с токеном из БД
        ...
        # Редирект на админ панель с успехом
