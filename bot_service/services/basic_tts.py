...
class BasicTTS:
        ...
        **kwargs
        ...
        try:
            ...
            if volume_level != 50.0:
                ...
                logger.info(f"🔊 Применена громкость: {volume_level}% ({db_change:+.1f}dB)")
            ...
            logger.info(f"💾 Базовая TTS: сохранено в WAV: {output_wav}")
            ...
            logger.error(f"❌ Ошибка при генерации аудио через gTTS: {e}", exc_info=True)
            return None
    ...
    def cleanup_old_files(self, max_age_seconds: int = 3600):
        ...
        try:
            ...
            if deleted_count > 0:
                logger.info(f"🗑️ Очищено {deleted_count} старых файлов базовой TTS")
