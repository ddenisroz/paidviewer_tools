...
from core.database import DropsHistory, DropsQuality, DropsConfig, UserStreak, MythicalDropsSession
...
class DropsService(DropsConfigMixin, DropsStreakMixin, DropsMythicalMixin):
    ...
    def get_drops_history(self, user_id: int = None, session_id: str = None, channel_name: str = None, platform: str = "twitch", limit: int = 50, offset: int = 0) -> List[DropsHistory]:
        ...
        return self.history_repo.get_history(channel_name, platform, user_id, session_id, limit, offset)
    ...
    def get_drops_stats(self, user_id: int = None, session_id: str = None, channel_name: str = None, platform: str = "twitch") -> Dict[str, Any]:
        ...
        total_drops = self.history_repo.count_drops(channel_name, platform, user_id, session_id)
        
        today = utcnow_naive().date()
        today_drops = self.history_repo.count_drops(channel_name, platform, user_id, session_id, after_date=today)
        
        legendary_drops = self.history_repo.count_legendary_drops(channel_name, platform, user_id, session_id)
        mythical_drops = self.history_repo.count_drops(channel_name, platform, user_id, session_id, lootbox_type="mythical")
    
    def get_full_channel_stats(self, user_id: int, channel_name: str, platform: str) -> Dict[str, Any]:
        ...
        streak = self.history_repo.count_drops(channel_name, platform, user_id=user_id, lootbox_type="streak")
        donation = self.history_repo.count_drops(channel_name, platform, user_id=user_id, lootbox_type="donation")
        mythical = self.history_repo.count_drops(channel_name, platform, user_id=user_id, lootbox_type="mythical")
        
        top_viewers_data = self.history_repo.get_top_viewers(user_id, channel_name, platform)
        ...
        return {
            ...
            "top_viewers": [{"viewer_name": v, "drops_count": c} for v, c in top_viewers_data]
        }

    # === Streak Methods ===
    
    def get_user_streaks_paginated(self, user_id: int, channel_name: str, platform: str = None, limit: int = 50, offset: int = 0) -> List[Dict]:
        """Get paginated user streaks."""
        streaks = self.history_repo.get_streaks_paginated(user_id, channel_name, platform, limit, offset)
        return [
            {
    ...
    def get_active_mythical_session(self, user_id: int = None, session_id: str = None, channel_name: str = None) -> Optional[Dict]:
        """Get active mythical drops session."""
        now = utcnow_naive()
        session = self.history_repo.get_active_mythical_session(channel_name, now, user_id, session_id)
