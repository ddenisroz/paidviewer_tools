# bot_service/services/admin/stats_service.py
"""
Сервис статистики для админ-панели.

Инкапсулирует все запросы к БД для получения статистики.
Следует Clean Architecture — контроллеры не имеют прямого доступа к БД.
"""
import logging
import shutil
import os
...
class AdminStatsService:
    ...
    def get_user_counts(self) -> Dict[str, int]:
        ...
        return {
            ...
            "active_today": self.repo.count_active_users_since(today_start),
            "active_week": self.repo.count_active_users_since(week_ago),
            "new_this_month": self.repo.count_new_users_since(month_start),
            "active": self.repo.count_users_by_status(is_active=True),
            "blocked": self.repo.count_users_by_status(is_blocked=True),
        }
    ...
    def get_message_counts(self) -> Dict[str, int]:
        ...
        return {
            "total": self.repo.count_messages(),
            "today": self.repo.count_messages(today_start),
            "week": self.repo.count_messages(week_ago),
            "month": self.repo.count_messages(month_start),
            "last_24h": self.repo.count_messages(day_ago),
            "last_1h": self.repo.count_messages(hour_ago),
        }
    ...
    def get_session_counts(self) -> Dict[str, int]:
        ...
        return {
            "active": self.repo.count_active_sessions(),
        }
    ...
    def get_tts_stats(self) -> Dict[str, int]:
        ...
        return {
            "enabled_users": self.repo.count_tts_enabled_users(),
        }
    ...
    def get_active_channels(self) -> list:
        ...
        return [
            {
                'id': ch.id,
                'channel_name': getattr(ch, 'channel_name', None),
                'vk_channel_name': getattr(ch, 'vk_channel_name', None),
                'platform': 'twitch' if getattr(ch, 'channel_name', None) else 'vk',
                'tts_enabled': getattr(ch, 'tts_enabled', False),
    ...
    def get_storage_stats() -> Dict[str, float]:
        ...
        # This logic interacts with file system, suitable for service or util, not repo.
        storage_used_gb = 0.0
    ...
    # === Aggregated Methods for Dashboard ===
    ...
    def get_dashboard_stats(self) -> Dict[str, Any]:
        ...
        return {
            "users": {
                ...
                "active_week": user_counts["active_week"],
                "new_this_month": user_counts["new_this_month"],
            },
            "tts": {
                "requests_today": message_counts["today"],
                "requests_week": message_counts["week"],
                "requests_month": message_counts["month"],
            },
            "system": {
                "errors_24h": error_counts["errors_24h"],
                "storage_used_gb": storage_stats["used_gb"],
                "storage_total_gb": storage_stats["total_gb"],
            }
        }
    ...
    def get_monitoring_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "messages": {
                "last_24h": message_counts["last_24h"],
                "last_1h": message_counts["last_1h"],
            },
            "sessions": session_counts,
            "integrations": {
                "active": integration_counts["total"],
                "twitch": integration_counts["twitch"],
                "vk": integration_counts["vk"],
            },
        }
    ...
    def get_analytics(self) -> Dict[str, Any]:
        ...
        return {
            "active_users": self.repo.count_active_users_since(hour_ago),
            "total_messages": self.repo.count_messages(),
            "recent_messages": self.repo.count_messages(hour_ago),
            "tts_requests": self.get_tts_stats()["enabled_users"],
    ...
    def get_admin_users_list(
        ...
        # Batch fetch tokens (avoid N+1)
        user_ids = [u.id for u in users]
        tokens = self.repo.get_active_tokens_for_users(user_ids)
        
        # Group tokens by user_id and platform
        tokens_map = {}
        for t in tokens:
            key = (t.user_id, t.platform)
            tokens_map[key] = t
        
        # Batch fetch whitelist status
        usernames = []
        for u in users:
        ...
        # Build response (Transformation logic stays in service)
        user_data = []
        for u in users:
            ...
            user_data.append({
                ...
                'integrations': {
                    'twitch': {'connected': twitch_connected, 'username': twitch_display, 'enabled': twitch_connected},
                    'vk': {'connected': vk_connected, 'username': vk_display, 'enabled': vk_connected}
                },
    ...
    def get_user_details(self, user_id: int) -> Optional[Dict[str, Any]]:
        ...
        return {
            "user": {
                ...
                'twitch_is_vip': getattr(user, 'twitch_is_vip', False),
                'twitch_is_subscriber': getattr(user, 'twitch_is_subscriber', False),
