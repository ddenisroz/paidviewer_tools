# tts_service/prometheus_metrics.py
import time
import asyncio
import logging
import torch
...
class TTSPrometheusMetrics:
    ...
    def __init__(self, port: int = 8003):
        ...
        # Счетчики TTS запросов
        self.tts_requests_total = Counter(
            'tts_requests_total',
            'Total number of TTS requests',
            ['voice', 'platform', 'status', 'processing_type']
        )
        
        self.tts_requests_by_user = Counter(
            'tts_requests_by_user_total',
            'TTS requests by user',
            ['user_id', 'platform']
        )
        
        # Время синтеза речи
        self.tts_synthesis_duration = Histogram(
            'tts_synthesis_duration_seconds',
            'Time spent on TTS synthesis',
            ['voice', 'platform', 'processing_type'],
            buckets=[0.1, 0.5, 1.0, 2.0, 5.0, 10.0, 30.0, 60.0, 120.0]
        )
        
        self.tts_synthesis_duration_summary = Summary(
            'tts_synthesis_duration_summary_seconds',
            'Summary of TTS synthesis time',
            ['voice', 'platform', 'processing_type']
        )
        ...
        # GPU Worker Pool метрики
        self.gpu_workers_active = Gauge(
            'gpu_workers_active',
            'Number of active GPU workers'
        )
        
        self.gpu_workers_concurrent = Gauge(
            'gpu_workers_concurrent',
            'Current concurrent GPU workers'
        )
        
        self.gpu_tasks_processed = Counter(
            'gpu_tasks_processed_total',
            'Total GPU tasks processed',
            ['status']
        )
        
        self.gpu_task_processing_time = Histogram(
            'gpu_task_processing_duration_seconds',
            'GPU task processing duration',
            ['task_type'],
        ...
        self.async_workers_active = Gauge(
            'async_workers_active',
            'Number of active async workers'
        )
        
        self.async_workers_total = Gauge(
            'async_workers_total',
            'Total number of async workers'
        )
        
        self.async_queue_size = Gauge(
            'async_queue_size',
            'Size of async worker queues',
            ['priority']
        )
        
        self.async_tasks_processed = Counter(
            'async_tasks_processed_total',
            'Total async tasks processed',
            ['status', 'processing_type']
        )
        
        self.async_task_processing_time = Histogram(
            'async_task_processing_duration_seconds',
            'Async task processing duration',
            ['priority', 'processing_type'],
            buckets=[0.1, 0.5, 1.0, 2.0, 5.0, 10.0, 30.0, 60.0]
        )
    ...
    def _monitoring_loop(self, interval: float):
        ...
        while self.running:
            try:
                # Обновляем GPU метрики
                self._update_gpu_metrics()
                
                # Обновляем системные метрики
                self._update_system_metrics()
                
                # Обновляем метрики воркеров
    ...
    def record_tts_synthesis_time(self, duration: float, voice: str, platform: str, processing_type: str = "cpu"):
        ...
        self.tts_synthesis_duration.labels(
            voice=voice, 
            platform=platform,
            processing_type=processing_type
        ).observe(duration)
        
        self.tts_synthesis_duration_summary.labels(
            voice=voice, 
            platform=platform,
            processing_type=processing_type
        ).observe(duration)
