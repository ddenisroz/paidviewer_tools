...
class TTSWorker:
    ...
    async def _process_task(self, task: Dict[str, Any]):
        ...
        try:
            ...
            if audio_url:
                # Публикуем результат через Redis Pub/Sub
                result = {
    ...
    async def _synthesize_speech(self, text: str, voice: str, user_id: int) -> Optional[str]:
        ...
        try:
            # Используем асинхронный TTS движок с конвертацией
            from tts_service.async_tts_engine import async_tts_engine
            from tts_service.async_audio_converter import async_audio_converter
            
            # Инициализируем асинхронные компоненты если нужно
            ...
            # Конвертируем в WAV для лучшего качества
