...
# Маппинг сокращений и переводов категорий
CATEGORY_ALIASES = {
    # === Популярные игры - сокращения ===
    ...
    'ark': ['ARK'],
    'poe': ['Path of Exile'],
    'fifa': ['FC 24', 'EA SPORTS FC 24', 'FIFA', 'FC 25'],
    ...
    'chat': ['Just Chatting', 'Говорим и смотрим'],
...
def calculate_relevance(category_name: str, query: str) -> float:
    ...
    if exact_word_matches == len(query_words):
        ...
        query_pattern = r'[\s\-:]+'.join(query_words)
    ...
    for query_word in query_words:
        for i, cat_word in enumerate(cat_words):
            ...
            # Запрос начинается со слова (для коротких запросов)
