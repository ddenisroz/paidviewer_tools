...
const dedupeRewards = (items: Reward[]): Reward[] => {
  ...
  for (const reward of items) {
    const qualityName = typeof reward.quality === 'object'
...
const RewardsManager: React.FC<RewardsManagerProps> = React.memo(({ user, channelName, onRewardsCountChange, integrations }) => {
  ...
  useEffect(() => {
    if (editorMode === 'edit' && selectedReward) {
      const qualityId = typeof selectedReward.quality === 'object'
        ? (selectedReward.quality?.id || null)
        : (typeof selectedReward.quality === 'number' ? selectedReward.quality : null);
                  ...
                  {qualityRewards.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2 lg:grid-cols-3">
                      {qualityRewards.map((reward) => {
                        const isSelected = editorMode === 'edit' && Number(reward.id) === selectedRewardId;
                        return (
                          <div
                            key={reward.id}
                          ...
                          >
                            ...
                            {reward.description && (
                              <p className="mb-2 line-clamp-2 text-xs text-muted-foreground">{reward.description}</p>
                            )}
        ...
        <Card className={`${SURFACE_CARD_CLASS} xl:sticky xl:top-20 h-fit`}>
          ...
          <CardContent className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2.5">
              <div className="space-y-1.5">
                ...
                <Select
                  ...
                  onValueChange={(value) => setForm((prev) => ({ ...prev, quality_id: parseInt(value, 10) || firstQualityId }))}
                >
                  <SelectTrigger id="reward_quality" className={CONTROL_TRIGGER_CLASS}>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent className={CONTROL_CONTENT_CLASS}>
                    {(qualitiesData.length > 0 ? (qualitiesData as unknown as QualityConfig[]) : QUALITIES).map((q) => {
                      const qName = typeof q.name === 'string' ? q.name : '';
                      const qualityInfo = QUALITIES.find((qual) => qual.name.toLowerCase() === qName.toLowerCase()) || q;
                      return (
                        <SelectItem key={q.id} value={q.id.toString()}>
                          <div className="flex items-center gap-2">
                            {'image' in qualityInfo && qualityInfo.image ? <img src={qualityInfo.image} alt={qualityInfo.label || qName} className="h-4 w-4" /> : null}
            ...
            <div className="flex flex-wrap gap-2 pt-2">
              ...
              {editorMode === 'edit' ? (
                <Button
                  onClick={() => setDeleteConfirmId((prev) => (prev ? null : Number(selectedReward?.id || 0)))}
                  variant="outline"
            ...
            {deleteConfirmId && editorMode === 'edit' && selectedReward && Number(selectedReward.id) === deleteConfirmId ? (
