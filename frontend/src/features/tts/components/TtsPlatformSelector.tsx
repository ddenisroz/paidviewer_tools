...
const TtsPlatformSelector: React.FC = () => {
  ...
  useEffect(() => {
    if (platformSettingsData) {
      ...
      const normalizedPlatforms = Array.isArray(data.enabled_platforms) ? data.enabled_platforms : ['twitch', 'vk'];
  ...
  const platforms: { id: PlatformId; name: string; icon: React.ComponentType<{ className?: string }>; description: string }[] = [
    {
      id: 'twitch',
      name: 'Twitch',
      icon: TwitchIcon,
  ...
  const styleMap: Record<PlatformId, {
    accentText: string;
    accentBg: string;
    accentBorder: string;
    pill: string;
  ...
  return (
    <div className="card-glass rounded-2xl border border-border">
        ...
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex items-center space-x-2">
            ...
            <button
              ...
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${ttsEnabled
                ? 'bg-primary'
                : 'bg-muted'
                } ${saving || isToggling ? 'opacity-50' : ''}`}
            >
              <span
                className={`inline-block h-4 w-4 transform rounded-full bg-background transition-transform ${ttsEnabled ? 'translate-x-6' : 'translate-x-1'
                  }`}
              />
            </button>
          </div>
      ...
      <div className="p-6 border-b border-border/60">
        <div className={`flex items-center p-4 rounded-lg border ${ttsEnabled
              ...
              >
                <div className="flex items-center space-x-3">
                  <div className={`p-2 rounded-lg ${isActive ? styles.accentBg : 'bg-muted'}`}>
                    <Icon className={`w-5 h-5 ${isActive ? styles.accentText : 'text-muted-foreground'}`} />
                  </div>
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <button
                    ...
                    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${isEnabled && ttsEnabled
                      ? styles.toggleOn
                      : 'bg-muted'
                      } ${saving || !ttsEnabled ? 'opacity-50 cursor-not-allowed' : ''}`}
                  >
                    <span
                      className={`inline-block h-4 w-4 transform rounded-full bg-background transition-transform ${isEnabled ? 'translate-x-6' : 'translate-x-1'
                        }`}
                    />
                  </button>
                </div>
