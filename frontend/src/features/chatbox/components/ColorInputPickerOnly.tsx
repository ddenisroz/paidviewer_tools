import React from 'react';

interface ColorInputPickerOnlyProps {
    value: string;
    onChange: (value: string) => void;
    label?: string;
}

const normalizeHex = (raw: string): string | null => {
    const trimmed = raw.trim();
    if (!trimmed) return null;
    const withHash = trimmed.startsWith('#') ? trimmed : `#${trimmed}`;
    if (/^#[0-9A-Fa-f]{3}$/.test(withHash)) {
        const r = withHash[1];
        const g = withHash[2];
        const b = withHash[3];
        return `#${r}${r}${g}${g}${b}${b}`.toUpperCase();
    }
    if (/^#[0-9A-Fa-f]{6}$/.test(withHash)) {
        return withHash.toUpperCase();
    }
    return null;
};

const ColorInputPickerOnly: React.FC<ColorInputPickerOnlyProps> = ({ value, onChange, label }) => {
    const normalizedValue = normalizeHex(value) || '#000000';

    return (
        <div className="space-y-2">
            {label && <span className="text-xs text-muted-foreground min-w-16">{label}</span>}
            <div className="rounded-lg border border-slate-700/70 bg-slate-950/45 p-2 flex items-center gap-2">
                <input
                    type="color"
                    value={normalizedValue}
                    onChange={(e) => {
                        const normalized = normalizeHex(e.target.value);
                        if (normalized) onChange(normalized);
                    }}
                    aria-label="Выбрать цвет"
                    className="h-9 w-14 shrink-0 cursor-pointer rounded-md border border-slate-600/80 bg-slate-900/90 p-0 [color-scheme:dark] [&::-webkit-color-swatch-wrapper]:p-[3px] [&::-webkit-color-swatch]:rounded-sm [&::-webkit-color-swatch]:border-none"
                />
                <span className="text-xs text-muted-foreground font-mono">{normalizedValue}</span>
            </div>
        </div>
    );
};

export default ColorInputPickerOnly;
