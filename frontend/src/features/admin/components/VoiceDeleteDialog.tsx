...
const MODAL_PANEL_CLASS = 'pointer-events-auto flex w-full max-w-md flex-col overflow-hidden rounded-xl border border-border/70 bg-background/95 shadow-2xl shadow-black/35';
      ...
      <div className="pointer-events-none fixed inset-0 z-[10000] flex items-center justify-center p-4">
        <div className={MODAL_PANEL_CLASS} onClick={event => event.stopPropagation()}>
          <div className="border-b border-border/70 p-4">
            ...
            <p className="mt-2 text-sm text-muted-foreground">Голос "{voice.name}" будет удалён без возможности восстановления.</p>
          </div>
          <div className={DIALOG_FOOTER_CLASS}>
            <Button onClick={onClose} variant="outline" className={`${DIALOG_ACTION_CLASS} border-border/70 bg-background/60 text-foreground hover:bg-accent/70`}>Отмена</Button>
            <Button onClick={onConfirm} variant="destructive" disabled={isDeleting} className={DIALOG_ACTION_CLASS}>{isDeleting ? 'Удаляю...' : 'Удалить голос'}</Button>
          </div>
        </div>
      </div>
    </>,
