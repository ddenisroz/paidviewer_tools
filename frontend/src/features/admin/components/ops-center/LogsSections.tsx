...
import { AdminEmptyState, ADMIN_CARD_CLASS } from '@/features/admin/components/admin-ui';
  ...
  <Card className={ADMIN_CARD_CLASS}>
    ...
    <CardContent className="grid gap-3 md:grid-cols-3">
      <Select value={days} onValueChange={onDaysChange}>
        <SelectTrigger className="border-border/70 bg-background/60"><SelectValue placeholder="Период" /></SelectTrigger>
        <SelectContent>
      ...
      <Select value={status} onValueChange={onStatusChange}>
        <SelectTrigger className="border-border/70 bg-background/60"><SelectValue placeholder="Статус" /></SelectTrigger>
        <SelectContent>
      ...
      <Select value={actionType} onValueChange={onActionTypeChange}>
        <SelectTrigger className="border-border/70 bg-background/60"><SelectValue placeholder="Тип действия" /></SelectTrigger>
        <SelectContent>
...
export const LogsContent: React.FC<{ data: LogsOverviewPayload; isLoading: boolean }> = ({ data, isLoading }) => (
  <div className="grid gap-4 xl:grid-cols-[1.2fr_1fr]">
    <Card className={ADMIN_CARD_CLASS}>
      ...
      <CardContent className="space-y-3">
        ...
        {!isLoading && (data.recent_admin_logs ?? []).length === 0 ? (
          <AdminEmptyState title="Логи не найдены" description="Попробуйте изменить период или снять фильтры." icon={FileText} className="border-none bg-transparent shadow-none" />
        ) : null}
        {!isLoading ? (data.recent_admin_logs ?? []).map(log => (
            ...
            <div className="flex items-start justify-between gap-3">
              <div className="flex min-w-0 flex-1 gap-3">
                ...
                <div className="min-w-0">
                  <p className="text-sm font-medium text-foreground">{log.description ?? 'Действие без описания'}</p>
                  ...
                  <p className="mt-1 flex items-center gap-1 text-xs text-muted-foreground">
                    ...
                    {log.timestamp ? new Date(log.timestamp).toLocaleString('ru-RU') : 'время неизвестно'}
                  </p>
                  {log.error_message ? <p className="mt-2 text-xs text-destructive">{log.error_message}</p> : null}
                </div>
              </div>
              <Badge variant="outline" className={logStatusBadgeClass(log.status)}>{log.status}</Badge>
            </div>
          </div>
        )) : null}
      </CardContent>
    ...
    <Card className={ADMIN_CARD_CLASS}>
      <CardHeader>
        <CardTitle className="flex items-center gap-2 text-base"><TerminalSquare className="h-4 w-4" />System log preview</CardTitle>
      </CardHeader>
      <CardContent>
        {(data.system_logs_preview ?? []).length === 0 ? (
          ...
          <div className="max-h-[420px] overflow-auto rounded-xl border border-border/70 bg-background/70 p-3">
            <pre className="whitespace-pre-wrap text-xs text-muted-foreground">{(data.system_logs_preview ?? []).join('\n')}</pre>
          </div>
        )}
      </CardContent>
    </Card>
