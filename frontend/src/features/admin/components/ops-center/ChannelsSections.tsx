...
export const BlockChannelCard: React.FC<{
  ...
  <Card className={ADMIN_CARD_CLASS}>
    ...
    <CardContent className="grid gap-3">
      <Input value={channelName} onChange={event => onChannelNameChange(event.target.value)} placeholder="Название канала" />
      <Textarea value={reason} onChange={event => onReasonChange(event.target.value)} placeholder="Причина блокировки (необязательно)" rows={3} />
  ...
  return (
    <div className="rounded-xl border border-border/70 bg-background/55 p-4">
      <div className="flex items-center justify-between gap-2">
        ...
        <Badge
          variant="outline"
          className={channel.tts_enabled ? 'border-emerald-500/30 bg-emerald-500/10 text-emerald-300' : 'border-border/70 bg-background/60 text-muted-foreground'}
        >
...
const BlockedChannelRow: React.FC<{ channel: BlockedChannel; onUnblock: (id: number) => void }> = ({ channel, onUnblock }) => (
  <div className="rounded-xl border border-border/70 bg-background/55 p-4">
    <div className="flex items-start justify-between gap-3">
      ...
      <Button variant="ghost" size="sm" className="h-8 w-8 p-0 text-destructive hover:bg-destructive/10 hover:text-destructive" onClick={() => onUnblock(channel.id)}>
    ...
    <Card className={ADMIN_CARD_CLASS}>
      ...
      <CardContent className="space-y-3">
        {activeChannels.length === 0 ? (
          ...
          activeChannels.map(channel => <ActiveChannelRow key={`${channel.platform}-${channel.id}`} channel={channel} />)
        )}
      </CardContent>
    </Card>
    <Card className={ADMIN_CARD_CLASS}>
      ...
      <CardContent className="space-y-3">
        {blockedChannels.length === 0 ? (
          ...
          blockedChannels.map(channel => <BlockedChannelRow key={channel.id} channel={channel} onUnblock={onUnblock} />)
        )}
      </CardContent>
    </Card>
  </div>
