...
export const OverviewStatsGrid: React.FC<{ data: OverviewPayload }> = ({ data }) => {
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      <AdminMetricCard title="Пользователи" value={stats.totalUsers} caption={`${stats.activeToday} активны сегодня`} icon={Users} />
      <AdminMetricCard title="TTS сегодня" value={stats.requestsToday} caption="запросов синтеза за текущие сутки" icon={Layers3} />
      <AdminMetricCard title="Воркеры" value={stats.totalWorkers} caption={`${stats.onlineWorkers} online, ${stats.offlineWorkers} offline`} icon={Bot} />
      <AdminMetricCard title="Whitelist" value={stats.whitelistTotal} caption={`${stats.activeChannels} активных каналов в системе`} icon={ShieldCheck} />
    </div>
  );
};
...
export const OverviewPlatformsCard: React.FC<{ data: OverviewPayload }> = ({ data }) => {
  ...
  return (
    <Card className={ADMIN_CARD_CLASS}>
      <CardHeader>
        <CardTitle className="text-base">Платформы и runtime</CardTitle>
      </CardHeader>
      <CardContent className="space-y-4">
        {platforms.map(([platform, config]) => (
          <RuntimePlatformCard key={platform} platform={platform} config={config} runtime={bots[platform]} token={tokens[platform]} />
        ))}
      </CardContent>
    </Card>
  );
    ...
    <div className="grid gap-4 xl:grid-cols-[1.15fr_1fr]">
      <Card className={ADMIN_CARD_CLASS}>
        ...
        <CardContent className="grid gap-3 md:grid-cols-2">
          {providers.map(([provider, health]) => <ProviderHealthCard key={provider} provider={provider} health={health} />)}
        </CardContent>
      </Card>
      <Card className={ADMIN_CARD_CLASS}>
        <CardHeader>
        ...
        <CardContent className="space-y-3">
          <AdminMetricCard title="Errors 24h" value={errors24h} />
          <AdminMetricCard title="Заблокированные каналы" value={blockedChannels} />
          <div className="rounded-xl border border-emerald-500/25 bg-emerald-500/10 p-4 text-sm text-emerald-200">
            <div className="flex items-start gap-2">
              <CheckCircle2 className="mt-0.5 h-4 w-4" />
              <span>Админ-центр работает через агрегированные read-model endpoints без ручного сбора состояния по всему фронту.</span>
            </div>
          </div>
        </CardContent>
      </Card>
