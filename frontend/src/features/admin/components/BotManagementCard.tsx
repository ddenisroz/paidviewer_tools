...
export const BotManagementCard: React.FC = () => {
  // TanStack Query hooks replace manual fetch functions
  const { data: tokenStatus, isLoading: loading } = useBotTokenStatusQuery();
  ...
  // Don't show card if not admin or no data
              ...
              <div className="flex items-start gap-2 p-3 rounded-lg bg-yellow-500/10 border border-yellow-500/20">
                ...
                <div className="flex-1 text-sm">
                  ...
                  <p className="text-muted-foreground mt-1">
                    Рекомендуется обновить токен вручную
                  </p>
                </div>
              </div>
            )}
