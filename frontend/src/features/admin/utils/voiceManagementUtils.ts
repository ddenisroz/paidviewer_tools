...
const extractVoicesArray = (payload: unknown): TtsVoice[] => {
  ...
  if (data.success && data.voices && typeof data.voices === 'object') return mergeVoiceLists(data.voices.global_voices, data.voices.user_voices);
  if (data.status === 'success') return [];
  if (Array.isArray(data.global_voices) || Array.isArray(data.user_voices)) return mergeVoiceLists(data.global_voices, data.user_voices);

  return [];
};

export const parseAdminVoicesResponse = (
  response: unknown,
): { voices: TtsVoice[]; warning: string | null } => {
