...
import type { UserSession, Integration, UsersResponse } from '../../../types/admin';
...
const UserManagementPage: React.FC = () => {
    ...
    const { data: usersResponse = { users: [], pagination: {} }, isLoading: usersLoading, refetch: loadUsers } = useQuery<UsersResponse>({
        ...
        queryFn: async () => {
            ...
            return (response.data as any) || { users: [], pagination: {} };
        },
        staleTime: 30 * 1000,
        refetchOnMount: true,
    ...
    const toggleWhitelistMutation = useMutation({
        ...
        onSuccess: (data: any, variables: { channelName: string; platform: 'twitch' | 'vk'; isWhitelisted: boolean; addBoth?: boolean }) => {
            queryClient.invalidateQueries({ queryKey: ['admin-users'] });
            queryClient.invalidateQueries({ queryKey: ['tts-status'] });
            queryClient.invalidateQueries({ queryKey: ['voices-whitelist-status'] });
