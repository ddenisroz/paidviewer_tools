...
import { BlockChannelCard, ChannelStatsGrid, ChannelsLists } from '@/features/admin/components/ops-center/ChannelsSections';
...
const AdminChannelsOpsScreen: React.FC = () => {
  ...
  if (isLoading) {
    return <div className="rounded-2xl border border-border/70 bg-card/70 p-6 text-sm text-muted-foreground">Загрузка каналов...</div>;
  }
  ...
  return (
    <div className="space-y-6">
      <AdminPageHeader
        ...
        actions={<Button variant="outline" size="sm" className={ADMIN_ACTION_BUTTON_CLASS} onClick={() => void refetch()} disabled={isFetching}>{isFetching ? 'Обновляю...' : 'Обновить'}</Button>}
      />
      ...
      />
      <ChannelsLists activeChannels={activeChannels} blockedChannels={data?.blocked_channels ?? []} onUnblock={channelId => void handleUnblockChannel(channelId)} />
    </div>
  );
};
