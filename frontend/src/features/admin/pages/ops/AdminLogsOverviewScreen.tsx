  ...
  return (
    <div className="space-y-6">
      <AdminPageHeader
        ...
        actions={<Button variant="outline" size="sm" className={ADMIN_ACTION_BUTTON_CLASS} onClick={() => void refetch()} disabled={isFetching}><RefreshCw className={`mr-2 h-4 w-4 ${isFetching ? 'animate-spin' : ''}`} />Обновить</Button>}
      />
      <LogsFiltersCard data={logs} days={days} status={status} actionType={actionType} onDaysChange={setDays} onStatusChange={setStatus} onActionTypeChange={setActionType} />
