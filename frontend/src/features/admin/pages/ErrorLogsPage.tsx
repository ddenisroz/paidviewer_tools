...
type FilterLevel = 'error' | 'warning' | 'all';
...
const ErrorLogsPage: React.FC = () => {
  ...
  const [filterLevel, setFilterLevel] = useState<FilterLevel>('error');
  ...
  const loadLogs = async (): Promise<void> => {
    try {
      ...
      if ((response.data as any)?.success) {
        let filtered = (response.data as any).logs || [];
        
        if (filterLevel !== 'all') {
          filtered = filtered.filter((log: string) => {
  ...
  const getLevelBadge = (logLine: string): LevelBadge => {
