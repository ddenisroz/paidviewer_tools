...
type CleanupType = 'logs' | 'cache' | 'backup';
...
const StorageManagementPage: React.FC = () => {
  ...
  const handleCleanup = async (type: CleanupType): Promise<void> => {
    const typeNames: Record<CleanupType, string> = {
      logs: 'Логи',
      cache: 'Кеш',
    ...
    try {
      ...
      if ((response.data as any)?.success) {
        ...
        const errorMsg = (response.data as any)?.error || (response.data as any)?.message || 'Неизвестная ошибка';
  ...
  const loadBackups = async (): Promise<void> => {
    try {
      ...
      if ((response.data as any)?.success && (response.data as any).data?.backups) {
        setBackups((response.data as any).data.backups);
      }
    } catch (error) {
