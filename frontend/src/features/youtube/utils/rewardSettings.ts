...
export const resolveYoutubeRewardState = (settings: YoutubeSettings): YoutubeRewardState => {
  ...
  const requestsRewardVkEnabled =
    settings.requests_reward_vk_enabled ?? (legacyEnabled && legacyPlatform === 'vk');
  const requestsRewardTwitchId =
    normalizeOptionalString(settings.requests_reward_twitch_id) ||
    (legacyPlatform === 'twitch' ? legacyId : '');
  const requestsRewardVkId =
    normalizeOptionalString(settings.requests_reward_vk_id) ||
    (legacyPlatform === 'vk' ? legacyId : '');
...
> => {
  ...
  return {
    requests_reward_enabled:
      rewardState.requestsRewardTwitchEnabled || rewardState.requestsRewardVkEnabled,
