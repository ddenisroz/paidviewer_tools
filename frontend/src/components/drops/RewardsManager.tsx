...
const RewardsManager: React.FC<RewardsManagerProps> = ({ user, channelName, onRewardsCountChange, integrations }) => {
  ...
  const getRewardsForQuality = (qualityName: string): Reward[] => {
    // ✅ Защита от null/undefined
    if (!rewards) return [];
  ...
  const getTotalWeight = (qualityName: string): number => {
