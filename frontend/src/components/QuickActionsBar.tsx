            ...
            if (channel === channelName) {
                ...
                if (streak_enabled !== undefined && eventPlatform && source === 'useDropsConfig') {
                    ...
                    setOptimisticStreakState(prev => {
                        ...
                        newState[statePlatformKey as keyof OptimisticStreakState] = streak_enabled;
                        return newState;
                    });
                    
                    // ✅ ОБНОВЛЯЕМ КЭШ: Кэш уже обновлен в useDropsConfig, но обновляем для консистентности
                    queryClient.setQueryData(queryKeys.drops.config(channelName), (old: any) => {
                        if (!old) return old;
                        const updated = {
                            ...old,
    ...
    const handleStreakToggle = () => {
        ...
        updateDropsConfigMutation.mutate(payload, {
            ...
            onError: (error) => {
                ...
                const previousConfig = queryClient.getQueryData(queryKeys.drops.config(channelName)) as any;
                if (previousConfig) {
                    setOptimisticStreakState({
