...
const Header = () => {
    ...
    useEffect(() => {
        const handleClickOutside = (event) => {
            if (integrationsOpen && !event.target.closest('.integrations-menu')) {
    ...
    const handleIntegrationToggle = async (platform) => {
        try {
            if (platform === 'twitch') {
    ...
    return (
        <header className="flex h-16 items-center justify-end gap-2 sm:gap-4 px-3 sm:px-6 lg:h-[70px] bg-transparent">
                    ...
                    {integrationsOpen && (
                        ...
                        >
                            <div className="p-4 space-y-3">
                                <h3 className="text-sm font-semibold text-muted-foreground">Платформы</h3>
            ...
            {user && (
                <Button
                    ...
                    title={isGuest ? "Отключиться" : "Выйти"}
                >
