...
import { Gift, Trophy, Star, Coins, Calendar, MessageSquare, TrendingUp, Check, X, Settings, BarChart3, Plus, Sparkles, LucideIcon } from 'lucide-react';
...
const LootboxSystem: React.FC<LootboxSystemProps> = ({ channelName }) => {
    ...
    const [lootboxSettings] = useState<LootboxSettings>({
        rarityRates: {
            common: 60,
            rare: 25,
            epic: 12,
    ...
    const [gameFieldData, setGameFieldData] = useState<GameDayData[]>(() => {
    ...
    const initializeImageLootboxes = () => {
        ...
        setImageLootboxes(mockLootboxes as ImageLootbox[]);
    };

    // Функция для обновления данных дня
    const updateDayData = (dayNumber: number, viewerName: string | null, isActive: boolean) => {
    ...
    const addViewerToDay = (dayNumber: number, viewerName: string) => {
        updateDayData(dayNumber, viewerName, true);
    };

    // Функция для удаления зрителя из дня
    const removeViewerFromDay = (dayNumber: number) => {
    ...
    const loadData = async (): Promise<void> => {
        try {
            ...
            const openingsData = openingsRes.data.data || openingsRes.data;
    ...
    const openLootbox = async (lootboxId: string | number): Promise<void> => {
        try {
            setOpeningLootboxId(lootboxId);
            const response = await lootboxService.openLootbox({ lootbox_id: typeof lootboxId === 'string' ? parseInt(lootboxId) : lootboxId });
            ...
            if (response.data?.success || data?.success) {
                ...
                if (result) {
                    ...
                    showLootboxResult(result as LootboxResult);
                    // Обновляем данные
                    loadData();
                }
            }
    ...
    const showLootboxResult = (result: LootboxResult) => {
    ...
    const getRewardIcon = (rewardType: string): LucideIcon => {
                    ...
                    <div className="bg-gray-800 rounded-lg p-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                            {imageLootboxes.map((lootbox) => {
                                const config = createLootboxImageConfig(lootbox as any, 'grid');
                                return (
                                    <div
                                        key={lootbox.id}
                ...
                <TabsContent value="donation-lootboxes" className="space-y-6">
                    <div className="bg-gray-800 rounded-lg p-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            ...
                            ] as DonationLootbox[]).map((lootbox) => (
                    ...
                    {lootboxResult && (
                        <div className="space-y-4 py-4">
                            ...
                            <div className="flex flex-col items-center justify-center space-y-3">
                                <div className={`text-6xl ${getRarityColor(lootboxResult.rarity || 'common')}`}>
                                    {React.createElement(getRewardIcon(lootboxResult.type || 'items'), { 
                                ...
                                <Badge 
                                    variant="outline" 
                                    className={`text-lg px-4 py-1 ${getRarityColor(lootboxResult.rarity || 'common')}`}
                                >
                                    {(lootboxResult.rarity || 'common').toUpperCase()}
                                </Badge>
                            </div>
