...
export function findMappedCategory(
  ...
  if (mappedName) {
    const mappedCategory = targetCategories.find((cat) => cat.name && cat.name.toLowerCase() === mappedName.toLowerCase());
    if (mappedCategory) return mappedCategory;
  }
  for (const [key, value] of Object.entries(categoryMapping)) {
    if (normalizeCategoryName(key) === normalizedInput) {
      const mappedCategory = targetCategories.find(
        (cat) => cat.name && (cat.name.toLowerCase() === value.toLowerCase() || normalizeCategoryName(cat.name) === normalizeCategoryName(value))
      );
      if (mappedCategory) return mappedCategory;
    }
  }
...
export function getSimilarCategories(
  ...
  const mappedName = categoryMapping[categoryName];
  if (mappedName) {
    const mapped = targetCategories.filter((cat) => cat.name && cat.name.toLowerCase().includes(mappedName.toLowerCase()));
    similar.push(...mapped);
  }
