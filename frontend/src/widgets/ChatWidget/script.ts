...
class ChatWidget {
  ...
  private connectWebSocket(): void {
    ...
    const fallbackBase = window.location.protocol === 'https:' ? `wss://${window.location.host}` : `ws://${window.location.host}`;
  ...
  private createRoleBadgeElement(role: string): HTMLSpanElement {
    ...
    const icons: Record<string, { viewBox: string; path: string }> = {
      broadcaster: { viewBox: '0 0 24 24', path: 'M12 2l3 6 6.5.9-4.7 4.6 1.1 6.5L12 17l-5.9 3.1 1.1-6.5L2.5 8.9 9 8z' },
      moderator: { viewBox: '0 0 24 24', path: 'M12 2l8 4v6c0 5-3.4 9.7-8 10-4.6-.3-8-5-8-10V6l8-4z' },
      vip: { viewBox: '0 0 24 24', path: 'M12 2l2.9 6.1L22 9.2l-5 4.9 1.2 6.9L12 17l-6.2 4 1.2-6.9-5-4.9 7.1-1.1z' },
