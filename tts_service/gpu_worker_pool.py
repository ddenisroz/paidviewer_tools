# tts_service/gpu_worker_pool.py
import asyncio
import logging
import time
import torch
...
class GPUWorkerPool:
    ...
    def __init__(
        ...
        if config:
            ...
            # Fallback на параметры по умолчанию
        ...
        self.stats = {
            ...
            'last_gpu_status': GPUStatus.IDLE
        }
    ...
    async def _monitor_gpu(self):
        ...
        while self.running:
            try:
                ...
                # Адаптируем concurrency
                await self._adapt_concurrency(metrics)
                
                # Логируем статус
                if metrics.memory_usage_percent > 0.5:  # Логируем только при значительной загрузке
                    logger.debug(f"GPU Status: {self.stats['last_gpu_status'].value}, "
    ...
    async def _get_gpu_metrics(self) -> GPUMetrics:
        ...
        try:
            ...
            try:
                ...
                # Fallback если pynvml не установлен
                gpu_utilization = 0.0
                temperature = None
                power_usage = None
    ...
    async def _adapt_concurrency(self, metrics: GPUMetrics):
        ...
        if metrics.memory_usage_percent >= self.gpu_memory_threshold_critical:
            # Критическая загрузка - минимальное количество воркеров
            self.current_concurrent = self.min_concurrent
        elif metrics.memory_usage_percent >= self.gpu_memory_threshold_high:
            # Высокая загрузка - уменьшаем количество воркеров
            self.current_concurrent = max(self.min_concurrent, self.current_concurrent - 1)
        elif metrics.memory_usage_percent <= self.gpu_memory_threshold_low:
            # Низкая загрузка - можно увеличить количество воркеров
            self.current_concurrent = min(self.max_concurrent, self.current_concurrent + 1)
        
        # Обновляем семафор если количество изменилось
        if self.current_concurrent != old_concurrent:
            ...
            logger.info(f"Adjusted concurrency: {old_concurrent} -> {self.current_concurrent} "
                       f"(GPU memory: {metrics.memory_usage_percent:.1%})")
    ...
    async def _update_semaphore(self):
        ...
        # Ждем освобождения старого семафора
    ...
    async def _worker(self, worker_name: str):
        ...
        while self.running:
            try:
                # Получаем задачу из Redis Stream
    ...
    def _get_next_task(self, worker_name: str):
        ...
        try:
            # Читаем из stream с блокировкой на 1 секунду
    ...
    async def _process_task(self, stream_id: str, fields: Dict[str, str], worker_name: str):
        ...
        try:
            ...
            # Используем семафор для ограничения concurrency
            async with self.semaphore:
                ...
                if result_path:
                    ...
                    # Отправляем результат обратно в Redis
    ...
    async def _send_result(self, task: SynthesisTask):
        ...
        try:
            result_data = {
                ...
                'status': 'completed' if task.result_path else 'failed',
            ...
            # Отправляем результат в stream результатов
            self.redis_client.xadd(
                f"{self.stream_name}_results",
                result_data
            )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self.stats,
            ...
            'device': self.device,
            'gpu_metrics': self._last_gpu_metrics.__dict__ if self._last_gpu_metrics else None
        }
