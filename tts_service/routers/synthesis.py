...
async def synthesize_channel(request: dict):
    ...
    try:
        ...
        # Используем Celery task для синтеза
        from tts_service.tasks import generate_tts_task
        from celery.result import AsyncResult
        import asyncio

        # Отправляем задачу в Celery
        # message_id генерируем или берем из request (но его там нет, так что генерируем)
        import uuid
        ...
        task = generate_tts_task.delay(
            ...
            voice=voice,
            user_id=user_id,
            platform="unknown", # channel_name usually implies platform context, but here we just need string
        ...
        # Ждем результата (синхронно для API клиента, но асинхронно для сервера если бы использовали async celery client,
        # но здесь мы блокируем поток, что не идеально, но сохраняет API контракт.
        # Для улучшения можно использовать polling или WebSocket, но это требует изменений в bot_service)
        ...
        for _ in range(max_retries):
            if task.ready():
                result = task.get() # Get result or raise exception
                break
        ...
        if result: # Result structure from task is dictionary
             # Task returns dict similar to what we need
             # Payload from task: type, audio_url, text, voice, etc.
             
             logger.info(f"[OK] [CHANNEL TTS] Синтез успешен для {channel_name} (Task {task.id})")
             
             return {
                "success": True,
                "selected_voice": result.get("voice", voice),
                "audio_url": result.get("audio_url"),
                "duration": 0, # Task might not return duration
                "tts_type": "f5" # Celery task uses F5
            }
        else:
