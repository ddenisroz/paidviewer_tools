# tts_service/tts_limits_service.py
import logging
import time
...
class TTSLimitsService:
    ...
    def __init__(self):
        ...
        self.global_gpu_time_limit = 300.0  # 5 минут в день
    ...
    def get_user_limits(self, user_id: int, db: Session) -> Dict[str, Any]:
        ...
        try:
            ...
            if not user:
                # Возвращаем глобальные лимиты для несуществующего пользователя
                return {
                    'max_text_length': self.global_max_text_length,
                    'daily_limit': self.global_daily_limit,
                    'gpu_time_limit': self.global_gpu_time_limit,
                    'priority_level': self.global_priority_level,
                    'tts_enabled': True
                }
            
            return {
                'max_text_length': user.tts_max_text_length or self.global_max_text_length,
                'daily_limit': user.tts_daily_limit or self.global_daily_limit,
                'gpu_time_limit': user.tts_gpu_time_limit or self.global_gpu_time_limit,
    ...
    def get_daily_usage(self, user_id: int, date: datetime.date, db: Session) -> Dict[str, Any]:
        ...
        try:
            ...
            usage = db.query(UserTTSUsage).filter(
                and_(
                    UserTTSUsage.user_id == user_id,
                    UserTTSUsage.date >= start_of_day,
                    UserTTSUsage.date <= end_of_day
                )
            ).first()
            ...
            return {
                ...
                'gpu_time_seconds': usage.gpu_time_seconds,
                ...
                'gpu_requests': usage.gpu_requests,
                'cpu_requests': usage.cpu_requests,
                'critical_requests': usage.critical_requests,
                'high_requests': usage.high_requests,
                'normal_requests': usage.normal_requests,
    ...
    def log_request(self, user_id: int, text: str, processing_time: float, 
                   processing_type: str, priority: int, success: bool, db: Session) -> bool:
        ...
        try:
            ...
            usage = db.query(UserTTSUsage).filter(
                and_(
                    UserTTSUsage.user_id == user_id,
                    UserTTSUsage.date >= start_of_day,
                    UserTTSUsage.date < start_of_day + timedelta(days=1)
                )
            ).first()
    ...
    def get_user_stats(self, user_id: int, days: int = 7, db: Session = None) -> Dict[str, Any]:
        ...
        try:
            ...
            stats = db.query(
                func.sum(UserTTSUsage.requests_count).label('total_requests'),
                func.sum(UserTTSUsage.gpu_time_seconds).label('total_gpu_time'),
                func.sum(UserTTSUsage.cpu_time_seconds).label('total_cpu_time'),
                func.sum(UserTTSUsage.total_characters).label('total_characters'),
                func.sum(UserTTSUsage.successful_requests).label('successful_requests'),
                func.sum(UserTTSUsage.failed_requests).label('failed_requests'),
                func.sum(UserTTSUsage.gpu_requests).label('gpu_requests'),
                func.sum(UserTTSUsage.cpu_requests).label('cpu_requests')
            ).filter(
                and_(
                    UserTTSUsage.user_id == user_id,
                    UserTTSUsage.date >= start_date,
                    UserTTSUsage.date <= end_date
                )
            ).first()
            ...
            return {
                ...
                'total_gpu_time': stats.total_gpu_time or 0.0,
                'total_cpu_time': stats.total_cpu_time or 0.0,
                ...
                'gpu_requests': stats.gpu_requests or 0,
                'cpu_requests': stats.cpu_requests or 0,
    ...
    def get_global_stats(self, days: int = 7, db: Session = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Агрегируем данные всех пользователей
            stats = db.query(
                func.count(func.distinct(UserTTSUsage.user_id)).label('unique_users'),
                func.sum(UserTTSUsage.requests_count).label('total_requests'),
                func.sum(UserTTSUsage.gpu_time_seconds).label('total_gpu_time'),
                func.sum(UserTTSUsage.cpu_time_seconds).label('total_cpu_time'),
                func.sum(UserTTSUsage.total_characters).label('total_characters'),
