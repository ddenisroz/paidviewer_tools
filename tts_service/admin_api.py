...
async def test_voice(
    ...
    try:
        ...
        # РСЃРїРѕР»СЊР·СѓРµРј РїРµСЂРµРґР°РЅРЅС‹Рµ РїР°СЂР°РјРµС‚СЂС‹ РёР»Рё Р·РЅР°С‡РµРЅРёСЏ РїРѕ СѓРјРѕР»С‡Р°РЅРёСЋ РёР· РіРѕР»РѕСЃР°
...
async def update_legacy_voice_settings(
    voice_id: int,
    settings: dict = Body(...),
    db: Session = Depends(get_db)
):
...
async def rename_legacy_voice(
    ...
    try:
        ...
        existing_voice = db.query(VoiceModel).filter(
            VoiceModel.name == sanitized_new_name,
...
async def rename_voice_endpoint(
    ...
    try:
        ...
        return {
            "success": True,
            "message": f"Voice renamed from '{old_name}' to '{sanitized_new_name}'",
