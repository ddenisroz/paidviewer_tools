# tts_service/gpu_integration.py
import asyncio
import logging
import json
import time
...
class GPUIntegrationService:
    ...
    async def _setup_consumer_groups(self):
        ...
        try:
            streams = [
                ...
                self.config.gpu_results_stream
            ]
    ...
    async def _start_workers(self):
        ...
        # Воркер для перенаправления задач на GPU
    ...
    async def _input_worker(self):
        ...
        while self.running:
            try:
                # Читаем задачи из основного stream
                messages = await asyncio.get_event_loop().run_in_executor(
                    None,
                    self._get_input_tasks
                )
    ...
    async def _process_input_task(self, stream_id: str, fields: Dict[str, str]):
        ...
        try:
            ...
            if use_gpu:
                # Перенаправляем на GPU Worker Pool
                await self._forward_to_gpu(task_id, task_data, stream_id)
            else:
                # Обрабатываем на CPU (fallback)
    ...
    async def _forward_to_gpu(self, task_id: str, task_data: Dict[str, Any], stream_id: str):
        ...
        try:
            # Отправляем задачу в GPU stream
            gpu_task_data = {
    ...
    async def _process_on_cpu(self, task_id: str, task_data: Dict[str, Any], stream_id: str):
        ...
        try:
            # Используем существующий TTS движок
            ...
            result_path = await tts_engine_manager.synthesize_speech_async(
                text=text,
                voice=voice,
                user_id=user_id
            )
    ...
    async def _result_worker(self):
        ...
        while self.running:
            try:
                # Читаем результаты из GPU results stream
                messages = await asyncio.get_event_loop().run_in_executor(
                    None,
                    self._get_gpu_results
                )
    ...
    async def _process_gpu_result(self, stream_id: str, fields: Dict[str, str]):
        ...
        try:
            ...
            if task_id in self.task_cache:
                ...
                # Подтверждаем обработку оригинальной задачи
    ...
    async def _send_result(self, task_id: str, result_path: Optional[str], error: Optional[str], stream_id: str):
        ...
        try:
            result_data = {
                ...
                'error': error or '',
                'processing_time': time.time() - self.task_cache.get(task_id, {}).get('started_at', time.time()),
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self.stats,
            'cached_tasks': len(self.task_cache),
            'running': self.running,
            'config': {
                'input_stream': self.config.input_stream,
                'output_stream': self.config.output_stream,
                'gpu_stream': self.config.gpu_stream,
