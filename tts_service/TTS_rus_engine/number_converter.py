...
class NumberToWordsConverter:
    ...
    def __init__(self):
        ...
        # Порядки (тысячи, миллионы, миллиарды)
        self.orders = [
            ("", "", ""),
            ("тысяча", "тысячи", "тысяч"),
        ...
        # Склонения для "один", "два", "три", "четыре"
    ...
    def convert_number(self, number: Union[int, str], gender: str = "m") -> str:
        ...
        try:
            ...
            for i, group in enumerate(groups):
                ...
                if i == 1:  # тысячи - женский род
                    current_gender = "f"
                elif i >= 2:  # миллионы и выше - мужской род
                    current_gender = "m"
    ...
    def convert_money(self, amount_str: str) -> str:
        ...
        try:
            ...
            currency_match = re.search(r'(руб|рубл|долл|евро|€|\$)', amount_str, re.IGNORECASE)
...
def convert_numbers_in_text(text: str) -> str:
    ...
    try:
        ...
        patterns = [
            ...
            (r'\b(\d{1,2}[:.]\d{2})\b', lambda m: number_converter.convert_time(m.group(1))),
            
            # Даты (DD.MM.YYYY или DD/MM/YYYY)
            (r'\b(\d{1,2}[./]\d{1,2}[./]\d{4})\b', lambda m: number_converter.convert_date(m.group(1))),
            
            # Денежные суммы (число + валюта)
            (r'\b(\d+(?:\.\d+)?)\s*(руб|рубл|долл|евро|€|\$)\b', lambda m: number_converter.convert_money(m.group(0))),
            
            # Простые числа (отдельно стоящие)
            (r'\b(\d+)\b', lambda m: number_converter.convert_number(m.group(1)))
        ]
        
        result = text
        
        # Применяем паттерны по порядку
...
if __name__ == "__main__":
    ...
    test_cases = [
        ...
        "1234.56 руб",
        "50 долл",
        "Сегодня 15 числа, время 14:30, сумма 1000 рублей"
    ]
