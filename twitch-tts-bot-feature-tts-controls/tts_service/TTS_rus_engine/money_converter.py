...
def convert_money_in_text(text: str) -> str:
    ...
    money_patterns = [
        # Рубли: 1000 руб, 1000 рублей, 1000 р.
        (r'\b(\d+(?:[.,]\d+)?)\s*(?:руб|рублей?|р\.?)\b', convert_rubles),
        # Доллары: $1000, 1000 долларов, 1000 долл.
        (r'\b\$?(\d+(?:[.,]\d+)?)\s*(?:долларов?|долл\.?|usd)\b', convert_dollars),
        # Евро: €1000, 1000 евро
        (r'\b€?(\d+(?:[.,]\d+)?)\s*(?:евро|eur)\b', convert_euros),
        # Копейки: 50 коп, 50 копеек
        (r'\b(\d+)\s*(?:коп|копеек?|коп\.?)\b', convert_kopecks),
        # Центы: 50 центов, 50 ц.
        (r'\b(\d+)\s*(?:цент|центов?|ц\.?)\b', convert_cents),
    ]
...
def format_number_with_currency(number: int, form1: str, form2: str, form5: str) -> str:
    ...
    # Формируем число словами
    number_word = format_number(number)
    
    # Определяем правильную форму валюты
...
def format_small_number(number: int) -> str:
    ...
    # Слова для десятков
    tens = {
        0: "", 10: "десять", 11: "одиннадцать", 12: "двенадцать", 13: "тринадцать",
...
def format_number(number: int) -> str:
    ...
    # Слова для сотен
    hundreds = {
        0: "", 100: "сто", 200: "двести", 300: "триста", 400: "четыреста",
    ...
    # Слова для тысяч
    thousands = {
        0: "", 1000: "одна тысяча", 2000: "две тысячи", 3000: "три тысячи",
    ...
    # Слова для миллионов
    millions = {
        0: "", 1000000: "один миллион", 2000000: "два миллиона", 3000000: "три миллиона",
        4000000: "четыре миллиона", 5000000: "пять миллионов", 6000000: "шесть миллионов",
        7000000: "семь миллионов", 8000000: "восемь миллионов", 9000000: "девять миллионов"
    }
    
    # Слова для миллиардов
    billions = {
        0: "", 1000000000: "один миллиард", 2000000000: "два миллиарда", 3000000000: "три миллиарда",
        4000000000: "четыре миллиарда", 5000000000: "пять миллиардов", 6000000000: "шесть миллиардов",
        7000000000: "семь миллиардов", 8000000000: "восемь миллиардов", 9000000000: "девять миллиардов"
    }
...
def convert_money_range_in_text(text: str) -> str:
    ...
    # Паттерн для диапазонов денежных сумм: 1000-2000 руб
    pattern = r'\b(\d+(?:[.,]\d+)?)-(\d+(?:[.,]\d+)?)\s*(?:руб|рублей?|р\.?)\b'
...
def convert_money_expressions_in_text(text: str) -> str:
    ...
    # Паттерны для выражений с денежными суммами
    patterns = [
        # "за 1000 руб" -> "за тысячу рублей"
        (r'\bза\s+(\d+(?:[.,]\d+)?)\s*(?:руб|рублей?|р\.?)\b', lambda m: f"за {format_rubles(float(m.group(1).replace(',', '.')))}"),
        # "до 5000 руб" -> "до пяти тысяч рублей"
        (r'\bдо\s+(\d+(?:[.,]\d+)?)\s*(?:руб|рублей?|р\.?)\b', lambda m: f"до {format_rubles(float(m.group(1).replace(',', '.')))}"),
        # "более 10000 руб" -> "более десяти тысяч рублей"
        (r'\bболее\s+(\d+(?:[.,]\d+)?)\s*(?:руб|рублей?|р\.?)\b', lambda m: f"более {format_rubles(float(m.group(1).replace(',', '.')))}"),
        # "менее 1000 руб" -> "менее тысячи рублей"
        (r'\bменее\s+(\d+(?:[.,]\d+)?)\s*(?:руб|рублей?|р\.?)\b', lambda m: f"менее {format_rubles(float(m.group(1).replace(',', '.')))}"),
    ]
