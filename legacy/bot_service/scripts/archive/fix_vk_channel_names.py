...
def fix_vk_channel_names():
    ...
    try:
        ...
        logger.info(f"📊 Найдено {len(vk_tokens)} VK токенов")
        
        for token in vk_tokens:
            try:
                user = db.query(User).filter(User.id == token.user_id).first()
                if not user:
                    logger.warning(f"⚠️ User {token.user_id} not found")
                    continue
                
                logger.info(f"\n🔍 Проверяем пользователя: {user.id}")
                    ...
                    if "data" in data:
                        ...
                        if channel_url:
                            ...
                            if channel_name != user.vk_username:
                                logger.info(f"   ✅ Обновляем: {user.vk_username} → {channel_name}")
                                user.vk_username = channel_name
                                db.commit()
                            else:
                                logger.info(f"   ℹ️ Ник канала уже корректный: {channel_name}")
                        else:
                            logger.warning(f"   ⚠️ channel.url отсутствует в API ответе")
                    else:
                ...
                logger.error(f"❌ Ошибка обработки токена {token.id}: {e}")
                continue
